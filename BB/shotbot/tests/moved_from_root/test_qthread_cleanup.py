...
def test_cleanup_implementation() -> bool:
    ...
    with open("previous_shots_model.py") as f:
        ...
        if (
            "def cleanup(self)" in content
            and "self._cleanup_worker_safely()" in content
        ):
    ...
    with open("shot_item_model.py") as f:
        ...
        cleanup_section = content[
            content.find("def cleanup(self)") : content.find("def cleanup(self)") + 2000
        ]
        ...
        if (
            "_thumbnail_timer.deleteLater()" in cleanup_section
            and "_thumbnail_timer = None" in cleanup_section
        ):
        ...
        if (
            "def deleteLater(self)" in content
            and "self.cleanup()" in content
            and "super().deleteLater()" in content
        ):
    ...
    with open("previous_shots_model.py") as f:
        ...
        if "self._worker = None" in content and "worker.deleteLater()" in content:
            ...
            cleanup_method = content[
                content.find("def _cleanup_worker_safely") : content.find(
