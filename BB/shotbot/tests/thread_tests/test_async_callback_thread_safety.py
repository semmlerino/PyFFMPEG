...
class TestShotItemModelThreadSafety:
    ...
    def test_concurrent_shot_lookup_thread_safety(self, thread_safe_model, test_shots):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(lookup_worker, chunk) for chunk in name_chunks]
    ...
    def test_shot_removal_during_concurrent_callbacks(
        ...
        for shot in test_shots:
            thread = threading.Thread(
                target=simulate_async_callback, args=(shot.full_name,)
            )
    ...
    def test_thumbnail_cache_concurrent_access(self, thread_safe_model, test_shots):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(cache_worker, worker_id) for worker_id in range(3)
            ]
            concurrent.futures.wait(futures, timeout=5.0)
    ...
    def test_model_reset_with_pending_callbacks(
        ...
        for shot in test_shots[:3]:  # Just a few to avoid overhead
            thread = threading.Thread(target=delayed_callback, args=(shot.full_name,))
            callback_threads.append(thread)
            thread.start()
...
class TestShotInfoPanelThreadSafety:
    ...
    def test_concurrent_pixmap_loading(self, thread_safe_panel, tmp_path, qtbot):
        ...
        for path in image_paths:
            ...
            loader.signals.loaded.connect(lambda img, p=path: loading_complete(True, p))
            loader.signals.failed.connect(lambda p=path: loading_error(p))
    ...
    def test_rapid_shot_changes_thread_safety(self, thread_safe_panel, tmp_path, qtbot, monkeypatch):
        ...
        assert (
            thread_safe_panel._current_shot in shots
