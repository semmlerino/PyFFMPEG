...
class TestQProcessErrorRecovery:
    ...
    def test_invalid_command_recovery(self, manager, qtbot):
        ...
        valid_id = manager.execute(
            command="echo", arguments=["recovery test"], capture_output=True
        )
    ...
    def test_permission_denied_recovery(self, manager, qtbot):
        ...
        try:
            ...
            process_id = manager.execute(
                command=script_path, capture_output=True, timeout_ms=5000
            )
            ...
            valid_id = manager.execute(
                command="echo", arguments=["permission recovery"], capture_output=True
            )
    ...
    def test_rapid_failure_recovery(self, manager, qtbot):
        ...
        success_id = manager.execute(
            command="echo", arguments=["rapid recovery"], capture_output=True
        )
    ...
    def test_timeout_recovery(self, manager, qtbot):
        ...
        normal_id = manager.execute(
            command="sleep", arguments=["1"], capture_output=False, timeout_ms=5000
        )
    ...
    def test_resource_exhaustion_recovery(self, manager, qtbot):
        ...
        new_pid = manager.execute(
            command="echo", arguments=["resource recovery"], capture_output=True
        )
    ...
    def test_invalid_working_directory_recovery(self, manager, qtbot):
        ...
        valid_id = manager.execute(
            command="echo", arguments=["directory recovery"], capture_output=True
        )
...
class TestConcurrentErrorHandling:
    ...
    def test_mixed_success_failure(self, manager, qtbot):
        ...
        for i in range(6):
            if i % 2 == 0:
                # Successful process
                pid = manager.execute(
                    command="echo", arguments=[f"success_{i}"], capture_output=True
                )
            else:
                # Failing process
                pid = manager.execute(
                    command="sh", arguments=["-c", f"exit {i}"], capture_output=False
                )
    ...
    def test_concurrent_timeouts(self, manager, qtbot):
        ...
        new_pid = manager.execute(
            command="echo", arguments=["post-timeout"], capture_output=True
        )
