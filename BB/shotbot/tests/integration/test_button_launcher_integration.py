...
class TestButtonLauncherIntegration:
    ...
    def test_each_button_launches_correct_app(self, main_window, mock_subprocess, qtbot):
    ...
    def test_button_state_changes_with_selection(self, main_window, qtbot):
        ...
        for app_name in Config.APPS:
            button = main_window.app_buttons[app_name]
            assert button.isEnabled(), f"{app_name} button should be enabled with shot selected"
        ...
        for app_name in Config.APPS:
            button = main_window.app_buttons[app_name]
            assert not button.isEnabled(), f"{app_name} button should be disabled without shot"
    ...
    def test_keyboard_shortcuts_launch_correct_apps(self, main_window, mock_subprocess, qtbot):
    ...
    def test_double_click_launches_default_app(self, main_window, mock_subprocess, qtbot):
    ...
    def test_custom_launcher_buttons_unique_commands(self, main_window, mock_subprocess, qtbot):
        ...
        for launcher in all_launchers[-3:]:  # Get the last 3 launchers we added
            ...
            if button and button.isVisible():
                ...
                if mock_subprocess.called:
                    ...
                    assert launcher.command in command_str or launcher.command.split()[0] in command_str, \
    ...
    def test_rapid_button_clicks(self, main_window, mock_subprocess, qtbot):
        ...
        for app_name in apps_to_test:
            ...
            command_count = sum(1 for cmd in all_commands 
                              if expected_command in str(cmd))
    ...
    def test_status_bar_shows_correct_app(self, main_window, mock_subprocess, qtbot):
        ...
        for app_name in Config.APPS:
            ...
            # Click button
            QTest.mouseClick(main_window.app_buttons[app_name], Qt.MouseButton.LeftButton)
            qtbot.wait(50)
            ...
            if status_message:
                ...
                assert app_name in status_message.lower() or \
                       app_name.upper() in status_message, \
...
class TestButtonCreationPatterns:
    ...
    def test_loop_created_buttons_have_unique_callbacks(self, qtbot):
        ...
        for app in test_apps:
            ...
            assert len(clicked_values) == 1, "Button should trigger exactly one callback"
