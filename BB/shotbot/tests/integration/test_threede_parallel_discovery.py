...
class TestParallelDiscoveryIntegration:
            ...
            for seq_num in ["seq001", "seq002"]:
                for shot_num in ["0010", "0020", "0030"]:
                    ...
                    for user in ["artist1", "artist2", "supervisor"]:
                        ...
                        (threede_dir / f"{show_name}_{seq_num}_{shot_num}_BG01.3de").write_text("# 3DE Scene\nversion 1.0")
    ...
    def test_find_all_3de_files_in_show_parallel_production_pattern(self):
        ...
        for file_tuple in results:
            assert len(file_tuple) == 6  # (file_path, show, sequence, shot, user, plate)
    ...
    def test_concurrent_parallel_discovery(self):
        ...
        threads = [
            threading.Thread(target=run_discovery, args=("TESTSHOW", testshow_shots[:3])),
            threading.Thread(target=run_discovery, args=("ANOTHERSHOW", anothershow_shots[:3]))
        ]
                ...
                for shot_num in range(5):  # 5 shots per sequence
                    ...
                    for user in ["lead", "artist"]:
                        ...
                        for plate in ["BG01", "FG01", "ROTO"]:
                            scene_file = user_dir / f"{show_name}_{seq_name}_{shot_name}_{plate}.3de"
                            scene_file.write_text(f"# 3DE Scene for {plate}\nversion 1.0")
