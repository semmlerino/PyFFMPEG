...
class TestProcessStateLifecycle:
    ...
    def test_normal_completion_state_transitions(self, manager, app, qtbot):
        ...
        process_id = manager.execute(
            command="echo", arguments=["test"], capture_output=True
        )
    ...
    def test_explicit_termination_state(self, manager, qtbot):
        ...
        process_id = manager.execute(
            command="sleep", arguments=["30"], capture_output=False
        )
    ...
    def test_failed_process_state(self, manager, qtbot):
        ...
        process_id = manager.execute(
            command="sh", arguments=["-c", "exit 42"], capture_output=False
        )
...
class TestStateConsistencyAcrossThreads:
    ...
    def test_worker_manager_state_sync(self, manager, qtbot):
        ...
        process_id = manager.execute(
            command="echo", arguments=["sync test"], capture_output=True
        )
    ...
    def test_concurrent_state_updates(self, manager, qtbot):
        ...
        for i in range(5):
            pid = manager.execute(
                command="echo", arguments=[f"test_{i}"], capture_output=True
            )
            if pid:
...
class TestStateTransitionEdgeCases:
    ...
    def test_state_persistence_across_refresh(self, manager, qtbot):
        ...
        # Start new process
        manager.execute(command="echo", arguments=["new"], capture_output=True)
...
class TestSignalEmissionDuringStateChanges:
    ...
    def test_signal_emission_sequence(self, manager, qtbot):
        ...
        # Execute command
        process_id = manager.execute(
            command="echo", arguments=["signal test"], capture_output=True
        )
        ...
        assert any(
            s[0] == "state" and s[1] == ProcessState.RUNNING for s in signals_received
        )
        assert any(
            s[0] == "state" and s[1] == ProcessState.FINISHED for s in signals_received
        )
    ...
    def test_failed_signal_emission(self, manager, qtbot):
        ...
        if final_info.error:
            assert (
                "timeout" in final_info.error.lower()
                or "timed out" in final_info.error.lower()
            )
