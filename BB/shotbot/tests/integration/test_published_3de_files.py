...
class TestPublished3DEFiles:
    ...
    def published_file_structure(self, tmp_path):
        ...
        publish_path = shot_path / "publish" / "mm" / "default" / "bg01" / "lens_distortion" / "v001" / "3de"
        publish_path.mkdir(parents=True)
        (publish_path / "019_JF_1080_mm-default_bg01_v001.3de").write_text("Published 3DE file")
        
        # Also create v002
        publish_path2 = shot_path / "publish" / "mm" / "default" / "bg01" / "lens_distortion" / "v002" / "3de"
        publish_path2.mkdir(parents=True)
        (publish_path2 / "019_JF_1080_mm-default_bg01_v002.3de").write_text("Published 3DE v2")
        ...
        publish2_path = shot2_path / "publish" / "mm" / "default" / "bg01" / "lens_distortion" / "v001" / "3de"
        publish2_path.mkdir(parents=True)
        (publish2_path / "008_AC_1320_mm-default_bg01_v001.3de").write_text("Published only")
    ...
    def test_extract_shot_info_from_published_path(self, published_file_structure):
        ...
        publish_path = Path("//shows/gator/shots/019_JF/019_JF_1080/publish/mm/default/bg01/lens_distortion/v001/3de/019_JF_1080_mm-default_bg01_v001.3de")
    ...
    def test_extract_shot_info_from_user_path(self):
        ...
        user_path = Path("/shows/jack_ryan/shots/GF_256/GF_256_0020/user/ryan-p/mm/3de/GF_256_0020_scene.3de")
    ...
    def test_find_published_files_in_shot(self, published_file_structure):
        ...
        assert len(all_files) == 3, f"Should find 3 files (2 published + 1 user), found {len(all_files)}"
        
        # Check that published files are included
        published_files = [f for f in all_files if "/publish/" in str(f)]
        assert len(published_files) == 2, f"Should find 2 published files, found {len(published_files)}"
    ...
    def test_published_files_not_excluded(self, published_file_structure):
        ...
        shots = [
            (str(published_file_structure["shot1_path"]), "gator", "019_JF", "019_JF_1080"),
            (str(published_file_structure["shot2_path"]), "gator", "008_AC", "008_AC_1320"),
        ]
        ...
        assert len(all_scenes) == 3, f"Should find 3 published scenes (john-d excluded), found {len(all_scenes)}"
        
        # All should be published files
        published_scenes = [s for s in all_scenes if s.user.startswith("published-")]
        assert len(published_scenes) == 3, f"All 3 scenes should be published, found {len(published_scenes)}"
