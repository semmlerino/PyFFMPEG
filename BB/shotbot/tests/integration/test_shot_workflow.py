...
workspace /shows/show2/shots/seq2/seq2_0030"""
    ...
    def execute_workspace_command(self, command: str, cache_ttl: int = 30, timeout: int = 120) -> str:
...
class _TestLauncherManager:
    ...
    def execute_launcher(self, launcher_id: str, custom_vars: Optional[Dict[str, str]] = None):
...
class TestShotDiscoveryWorkflow:
    ...
    def test_shot_discovery_to_cache_workflow(self, temp_cache_dir, mock_process_pool_manager):
    ...
    def test_cache_persistence_workflow(self, temp_cache_dir, mock_process_pool_manager):
...
class TestThumbnailWorkflow:
    ...
    def test_thumbnail_discovery_workflow(self, temp_cache_dir, mock_filesystem):
        ...
        cache_manager = CacheManager(cache_dir=temp_cache_dir)
        shot = Shot("testshow", "101_ABC", "0010", 
        ...
        thumbnails = list(FileUtils.find_files_by_extension(thumb_path, [".jpg", ".jpeg"]))
        assert len(thumbnails) == 2  # frame.1001.jpg and frame.1002.jpg
        
        # Cache thumbnail  
        with patch('cache_manager.QImage') as mock_qimage, \
             patch('cache_manager.QApplication') as mock_qapp, \
             patch('cache_manager.QThread') as mock_qthread, \
    ...
    def test_thumbnail_cache_retrieval(self, temp_cache_dir):
        ...
        retrieved = cache_manager.get_cached_thumbnail(shot.show, shot.sequence, shot.shot)
...
class TestLauncherWorkflow:
    ...
    def test_launcher_with_shot_context(self, tmp_path):
        ...
        shot = Shot("testshow", "101_SEQ", "0010", "/shows/testshow/shots/101_SEQ/101_SEQ_0010")
...
class TestErrorHandlingWorkflow:
    ...
    def test_cache_corruption_recovery(self, temp_cache_dir):
        ...
        assert result.success is True
        assert result.has_changes is True  # Should have changes since we started with empty
        assert len(model.get_shots()) == 1
