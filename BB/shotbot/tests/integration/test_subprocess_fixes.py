...
class TestSubprocessCreation(unittest.TestCase):
    ...
    def test_multiple_sessions_create_without_hanging(self):
        ...
        for i in range(3):
            ...
            self.assertTrue(session._is_alive(), 
                          f"Session {i} died immediately after creation")
    ...
    def test_strip_escape_sequences_method(self):
        ...
        for input_text, expected in test_cases:
            ...
            self.assertEqual(result, expected, 
                           f"Failed to strip: {repr(input_text)}")
        ...
        for cmd, expected in commands:
            ...
            if expected:
                self.assertIn(expected, result, 
                             f"Command '{cmd}' didn't return expected output")
...
shot003 /shows/test/shots/shot003"""
    ...
    def test_file_operations(self):
        ...
        try:
            ...
            result = self.pool.execute_workspace_command(f"cat {temp_path}", 
                                                        cache_ttl=0, timeout=5)
            self.assertIn("test content", result, "File content not read correctly")
            
            # List file
            result = self.pool.execute_workspace_command(f"ls -la {temp_path}", 
                                                        cache_ttl=0, timeout=5)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for cmd in commands:
                future = executor.submit(
                    self.pool.execute_workspace_command, 
                    cmd, 0, 10
                )
...
class TestCaching(unittest.TestCase):
    ...
    def test_cache_invalidation(self):
        ...
        self.assertNotEqual(result1, result3, "Should get new result after invalidation")
