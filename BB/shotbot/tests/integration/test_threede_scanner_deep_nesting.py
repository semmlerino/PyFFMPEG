...
class Test3DEScennerDeepNesting:
    ...
    def deep_shot_structure(self, tmp_path):
        ...
        return {
            "root": show_root,
            "show_root": show_root,
            "jack_ryan_shot": show_root / "jack_ryan" / "shots" / "GF_256" / "GF_256_0020",
    ...
    def test_find_deeply_nested_3de_files(self, deep_shot_structure):
        ...
        assert "excluded-user" not in users_found, "Should not find excluded user's files"
        ...
        assert len(ryan_scenes) == 3, f"Should find 3 files for ryan-p, found {len(ryan_scenes)}"
    ...
    def test_common_pattern_optimization(self, deep_shot_structure, monkeypatch, caplog):
        ...
        assert any("Checking common path" in record.message for record in caplog.records), \
            "Should check common paths first"
        assert any("mm/3de" in record.message or "matchmove/3de" in record.message 
                  for record in caplog.records), "Should check known patterns"
    ...
    def test_progressive_scan_batching(self, deep_shot_structure, monkeypatch):
        ...
        assert len(batches_received) >= 2, f"Should receive multiple batches with batch_size=2, got {len(batches_received)}"
        assert total_scenes == 3, f"Should find 3 total scenes for ryan-p, found {total_scenes}"
    ...
    def test_multiple_shows(self, deep_shot_structure):
        ...
        # Count total scenes
        assert len(all_scenes) >= 7, f"Should find at least 7 scenes total, found {len(all_scenes)}"
    ...
    def test_empty_directories(self, deep_shot_structure):
        ...
        scenes = ThreeDESceneFinder.find_scenes_for_shot(
            ...
            sequence="GF_256",
            shot="0020",
            excluded_users={"ryan-p", "kate-b", "john-d", "excluded-user"}  # Only check alice-w
        )
