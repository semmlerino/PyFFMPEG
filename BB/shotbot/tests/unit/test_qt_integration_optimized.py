...
class TestQtIntegration:
    ...
    def test_widget_lifecycle_integration(self, real_cache_manager, qtbot):
        ...
        # Use model
        mock_pool = Mock()
        mock_pool.execute_workspace_command.return_value = "workspace /lifecycle/test/0010"
...
class TestPerformanceValidation:
    ...
    def test_startup_time_improvement_validation(self, real_cache_manager):
        ...
        model.cache_manager.cache_shots([
            {"show": "PERF", "sequence": "seq01", "shot": "0010", "workspace_path": "/perf/path"}
        ])
        ...
        assert avg_time < 0.1, f"Average startup time {avg_time:.3f}s exceeds 0.1s target"
    ...
    def test_memory_usage_optimization(self, real_cache_manager):
        ...
        for i in range(50):
            # Add cache data
            model.cache_manager.cache_shots([
                {"show": f"MEM{i}", "sequence": "seq", "shot": "0010", "workspace_path": f"/mem/{i}"}
            ])
    ...
    def test_background_load_efficiency(self, real_cache_manager, qtbot):
        ...
        assert immediate_time < 0.1, f"Background loads blocked for {immediate_time:.3f}s"
        
        # Wait for background processing to complete
        qtbot.waitUntil(lambda: not model._loading_in_progress, timeout=3000)
        
        total_time = time.perf_counter() - start_time
        print(f"Background efficiency: immediate={immediate_time:.3f}s, total={total_time:.3f}s")