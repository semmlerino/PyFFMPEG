...
class MockWorker(ThreadSafeWorker):
    ...
    def __init__(self, worker_id: str = None, mock_state: WorkerState = WorkerState.RUNNING):
        super().__init__()
...
class TestLauncherManagerThreading:
    ...
    def test_cleanup_scheduling_prevents_duplicates(self, launcher_manager):
        ...
        with patch.object(manager, '_cleanup_retry_timer') as mock_timer:
            ...
            manager._schedule_cleanup_after_delay(ThreadingConfig.CLEANUP_INITIAL_DELAY_MS)
            manager._schedule_cleanup_after_delay(ThreadingConfig.CLEANUP_INITIAL_DELAY_MS)
            manager._schedule_cleanup_after_delay(ThreadingConfig.CLEANUP_INITIAL_DELAY_MS)
    ...
    def test_cleanup_finished_workers_atomic_checking(self, launcher_manager):
        ...
        workers = {
            ...
            "inconsistent_1": MockWorker(mock_state=WorkerState.STOPPED),  # Will be marked inconsistent
        }
        ...
        with patch.object(manager, '_check_worker_state_atomic', 
                         wraps=manager._check_worker_state_atomic) as mock_atomic:
            # Run cleanup
    ...
    def test_threadingconfig_integration(self, launcher_manager):
        ...
        assert manager.MAX_CONCURRENT_PROCESSES == ThreadingConfig.MAX_WORKER_THREADS * 25
        assert manager.CLEANUP_INTERVAL_MS == ThreadingConfig.CACHE_CLEANUP_INTERVAL * 1000
        assert manager.PROCESS_STARTUP_TIMEOUT_MS == ThreadingConfig.SUBPROCESS_TIMEOUT * 1000
    ...
    def test_worker_cleanup_timeout_values(self, launcher_manager):
        ...
        with patch.object(mock_worker, 'safe_stop', return_value=True) as mock_safe_stop:
            manager._remove_worker_safe(worker_key)
            
            # Verify safe_stop was called with ThreadingConfig timeout
            mock_safe_stop.assert_called_once_with(timeout_ms=ThreadingConfig.WORKER_STOP_TIMEOUT_MS)
    ...
    def test_schedule_cleanup_delay_parameter(self, launcher_manager):
        ...
        with patch.object(manager, '_cleanup_retry_timer') as mock_timer:
            ...
            mock_timer.setInterval.assert_called_once_with(ThreadingConfig.CLEANUP_INITIAL_DELAY_MS)
    ...
    @pytest.mark.parametrize("worker_state,expected_category", [
    ...
    def test_worker_categorization_logic(self, launcher_manager, worker_state, expected_category):
        ...
        if expected_category == "finished":
            ...
            if worker_state in [WorkerState.STOPPED, WorkerState.DELETED, WorkerState.ERROR]:
