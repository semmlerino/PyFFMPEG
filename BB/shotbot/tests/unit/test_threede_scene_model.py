...
def sample_scene():
    ...
    return ThreeDEScene(
        ...
        user="otheruser",
        plate="FG01",
        scene_path=Path("/shows/test_show/seq01/seq01_shot01/user/otheruser/work/3de/scenes/test.3de")
    )
...
def sample_scenes():
    ...
    scenes = [
        ThreeDEScene(
            ...
            user="user1",
            plate="FG01",
            scene_path=Path("/shows/test_show/seq01/seq01_shot01/user/user1/work/3de/scenes/test1.3de")
        ),
        ThreeDEScene(
            show="test_show",
            sequence="seq01",
            ...
            user="user2",
            plate="BG01",
            scene_path=Path("/shows/test_show/seq01/seq01_shot02/user/user2/work/3de/scenes/test2.3de")
        ),
        ThreeDEScene(
            show="test_show",
            sequence="seq02",
            ...
            user="user3",
            plate="EL01",
            scene_path=Path("/shows/test_show/seq02/seq02_shot01/user/user3/work/3de/scenes/test3.3de")
        ),
    ]
    return scenes
...
def duplicate_scenes():
    ...
    scenes = [
        ...
        ThreeDEScene(
            ...
            user="user1",
            plate="FG01",
            scene_path=Path("/shows/test_show/seq01/seq01_shot01/user/user1/work/3de/scenes/old.3de")
        ),
        ThreeDEScene(
            show="test_show",
            sequence="seq01",
            ...
            user="user2",
            plate="BG01",
            scene_path=Path("/shows/test_show/seq01/seq01_shot01/user/user2/work/3de/scenes/new.3de")
        ),
        ...
        ThreeDEScene(
            ...
            user="user3",
            plate="EL01",
            scene_path=Path("/shows/test_show/seq02/seq02_shot01/user/user3/work/3de/scenes/test.3de")
        ),
    ]
    return scenes
...
class TestThreeDEScene:
    ...
    def test_get_thumbnail_path_editorial_found(self, sample_scene):
        ...
        with patch.object(PathUtils, 'validate_path_exists') as mock_validate:
            with patch('threede_scene_model.FileUtils.get_first_image_file') as mock_get_image:
    ...
    def test_get_thumbnail_path_turnover_fallback(self, sample_scene):
        ...
        with patch.object(PathUtils, 'validate_path_exists') as mock_validate:
            with patch('threede_scene_model.FileUtils.get_first_image_file') as mock_get_image:
                with patch.object(PathUtils, 'find_turnover_plate_thumbnail') as mock_turnover:
                    # Editorial not found
    ...
    def test_get_thumbnail_path_publish_fallback(self, sample_scene):
        ...
        with patch.object(PathUtils, 'validate_path_exists') as mock_validate:
            with patch('threede_scene_model.FileUtils.get_first_image_file') as mock_get_image:
                with patch.object(PathUtils, 'find_turnover_plate_thumbnail') as mock_turnover:
                    with patch.object(PathUtils, 'find_any_publish_thumbnail') as mock_publish:
...
class TestThreeDESceneModel:
    ...
    def test_deduplicate_scenes_by_shot(self, mock_cache_manager, duplicate_scenes):
        ...
        seq01_shot01_scenes = [s for s in deduplicated if s.sequence == "seq01" and s.shot == "shot01"]
    ...
    def test_select_best_scene_priority(self, mock_cache_manager):
        ...
        scenes = [
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user1", plate="EL01",
                scene_path=Path("/test/el.3de")
            ),
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user2", plate="FG01",
                scene_path=Path("/test/fg.3de")
            ),
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user3", plate="BG01",
    ...
    def test_select_best_scene_mtime_wins(self, mock_cache_manager):
        ...
        scenes = [
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user1", plate="FG01",
                scene_path=Path("/test/older_fg.3de")
            ),
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user2", plate="EL01",
    ...
    def test_refresh_scenes_success(self, mock_cache_manager, sample_shots, sample_scenes):
    ...
    def test_refresh_scenes_no_changes(self, mock_cache_manager, sample_shots, sample_scenes):
    ...
    def test_refresh_scenes_with_deduplication(self, mock_cache_manager, sample_shots, duplicate_scenes):
        """Test that refresh applies deduplication."""
        model = ThreeDESceneModel(cache_manager=mock_cache_manager, load_cache=False)
        
        with patch('threede_scene_finder.ThreeDESceneFinder') as MockFinder:
            MockFinder.find_all_scenes_in_shows_efficient.return_value = duplicate_scenes
    ...
    def test_refresh_scenes_error_handling(self, mock_cache_manager, sample_shots):
        ...
        with patch('threede_scene_finder.ThreeDESceneFinder') as MockFinder:
            MockFinder.find_all_scenes_in_shows_efficient.side_effect = Exception("Test error")
    ...
    def test_refresh_scenes_always_caches(self, mock_cache_manager, sample_shots, sample_scenes):
        ...
        with patch('threede_scene_finder.ThreeDESceneFinder') as MockFinder:
            ...
            # Spy on cache method
            with patch.object(mock_cache_manager, 'cache_threede_scenes', wraps=mock_cache_manager.cache_threede_scenes) as spy:
                # First refresh
    ...
    def test_scenes_sorting(self, mock_cache_manager, sample_shots):
        ...
        unsorted_scenes = [
            ThreeDEScene(
                show="test", sequence="seq02", shot="shot01",
                workspace_path="/test", user="user2", plate="FG01",
                scene_path=Path("/test/2.3de")
            ),
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot01",
                workspace_path="/test", user="user1", plate="BG01",
                scene_path=Path("/test/1.3de")
            ),
            ThreeDEScene(
                show="test", sequence="seq01", shot="shot02",
                workspace_path="/test", user="user1", plate="EL01",
