...
class TestParametrizedPriority:
    ...
    def test_plate_priority_ordering(self, tmp_path, plate_name, priority):
        ...
        plate_path = (
            shows_root / "test" / "shots" / "seq01" / "seq01_0010" /
            "publish" / "turnover" / "plate" / plate_name / "v001" / "exr" / "4k"
        )
        ...
        other_path = plate_path.parent.parent.parent / other_name / "v001" / "exr" / "4k"
...
class TestParametrizedFileOperations:
    ...
    @pytest.mark.parametrize("file_size_mb,should_resize", [
...
class TestParametrizedPaths:
    ...
    @pytest.mark.parametrize("depth,exists", [
        (1, True),
        (5, True),
        (10, True),
...
class TestParametrizedCaching:
    ...
    @pytest.mark.parametrize("wait,expected_return", [
...
class TestParametrizedErrorHandling:
    ...
    @pytest.mark.parametrize("missing_component", [
        "show", "sequence", "shot", "workspace_path"
    ])
...
class TestParametrizedIntegration:
    ...
    def test_complete_discovery_scenarios(self, tmp_path, scenario):
        ...
        if scenario == "editorial_jpg":
            ...
            plate = shot_dir / "publish" / "turnover" / "plate" / "FG01" / "v001" / "exr" / "4k"
            plate.mkdir(parents=True)
            (plate / "fg.1001.exr").touch()
            expected_suffix = ".exr"
            
        elif scenario == "turnover_bg":
            plate = shot_dir / "publish" / "turnover" / "plate" / "BG01" / "v001" / "exr" / "4k"
            plate.mkdir(parents=True)
