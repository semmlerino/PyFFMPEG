...
class TestCustomLauncher:
    ...
    def test_custom_launcher_serialization_roundtrip(self):
        ...
        assert (
            restored.environment.command_prefix == original.environment.command_prefix
        )
        ...
        assert (
            restored.validation.check_executable == original.validation.check_executable
        )
        assert restored.validation.required_files == original.validation.required_files
        assert (
            restored.validation.forbidden_patterns
            == original.validation.forbidden_patterns
        )
...
class TestLauncherManager:
    ...
    def test_execute_launcher_with_test_pool(self, tmp_path, qtbot):
        ...
        manager.execution_finished.connect(
            lambda id, success: finished_signals.append((id, success))
        )
    ...
    def test_execute_nonexistent_launcher(self, tmp_path):
        ...
        result = manager.execute_launcher(
            "nonexistent-id", {"shot_name": shot.full_name}
        )
    ...
    def test_concurrent_launcher_execution(self, tmp_path):
        ...
        def execute_launcher():
            success = manager.execute_launcher(
                launcher_id, {"shot_name": shot.full_name}
            )
            if success:
