...
class TestWorkspaceScriptDirectory:
    ...
    def test_get_workspace_script_directory_default_user(self, workspace_path):
        ...
        expected = Path(workspace_path) / "user" / "testuser" / "mm" / "nuke" / "scripts" / "mm-default" / "scene" / "PL01"
    ...
    def test_get_workspace_script_directory_explicit_user(self, workspace_path):
        ...
        expected = Path(workspace_path) / "user" / "customuser" / "mm" / "nuke" / "scripts" / "mm-default" / "scene" / "PL01"
        assert script_dir == expected
        assert script_dir.exists()
    ...
    def test_get_workspace_script_directory_custom_params(self, workspace_path):
        ...
        expected = Path(workspace_path) / "user" / "testuser" / "mm" / "nuke" / "scripts" / "custom-plate" / "scene" / "PL02"
        assert script_dir == expected
        assert script_dir.exists()
...
class TestFindLatestNukeScript:
    ...
    def test_find_latest_script_custom_plate_and_pass(self, tmp_path, shot_name):
        ...
        for version in [1, 2, 3]:
            script = script_dir / f"{shot_name}_custom-plate_PL02_scene_v{version:03d}.nk"
            script.write_text(f"# Version {version}")
    ...
    def test_find_latest_script_handles_invalid_version_format(self, tmp_path, shot_name):
        ...
        (script_dir / f"{shot_name}_mm-default_PL01_scene_vXXX.nk").write_text("# invalid")
        (script_dir / f"{shot_name}_mm-default_PL01_scene_v1.nk").write_text("# wrong padding")
...
class TestEdgeCasesAndIntegration:
    ...
    def test_very_high_version_numbers(self, tmp_path, shot_name):
        ...
        high_version = 999
        script = script_dir / f"{shot_name}_mm-default_PL01_scene_v{high_version:03d}.nk"
