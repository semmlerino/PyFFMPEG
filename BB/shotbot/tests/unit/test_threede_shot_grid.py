...
class TestThreeDEShotGridColumnCalculation:
    ...
    def test_get_column_count_calculated(self, threede_grid):
        ...
        # Mock viewport width
        viewport_width = 500
        with patch.object(threede_grid.scroll_area.viewport(), 'width', return_value=viewport_width):
            count = threede_grid._get_column_count()
        
        expected = max(1, viewport_width // (Config.DEFAULT_THUMBNAIL_SIZE + Config.THUMBNAIL_SPACING))
        assert count == expected
    
    def test_get_column_count_minimum(self, threede_grid):
        """Test minimum column count is 1."""
        with patch.object(threede_grid.scroll_area.viewport(), 'width', return_value=10):
            count = threede_grid._get_column_count()
        assert count >= 1
...
class TestThreeDEShotGridKeyboardNavigation:
    ...
    def test_ensure_widget_visible(self, threede_grid, sample_scenes):
        ...
        with patch.object(threede_grid.scroll_area, 'ensureWidgetVisible') as mock_ensure:
            threede_grid.keyPressEvent(event)
