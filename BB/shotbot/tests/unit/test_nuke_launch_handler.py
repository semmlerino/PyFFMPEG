...
class TestNukeLaunchHandler:
    ...
    def test_prepare_nuke_command_basic(self, nuke_handler, mock_shot):
        ...
        command, messages = nuke_handler.prepare_nuke_command(
            mock_shot,
            "nuke",
            {}
        )
    ...
    def test_handle_workspace_scripts_open_latest(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.workspace_manager, 'get_workspace_script_directory') as mock_get_dir:
            with patch.object(nuke_handler.workspace_manager, 'find_latest_nuke_script') as mock_find_latest:
                mock_get_dir.return_value = Path("/test/workspace/comp/nuke")
                mock_find_latest.return_value = Path("/test/workspace/comp/nuke/TEST_0010_v001.nk")
    ...
    def test_handle_workspace_scripts_create_new(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.workspace_manager, 'get_workspace_script_directory') as mock_get_dir:
            with patch.object(nuke_handler.workspace_manager, 'get_next_script_path') as mock_get_next:
                with patch.object(nuke_handler, '_create_new_workspace_script') as mock_create:
                    mock_get_dir.return_value = Path("/test/workspace/comp/nuke")
                    mock_get_next.return_value = ("/test/workspace/comp/nuke/TEST_0010_v002.nk", 2)
                    ...
                    assert any("Creating new Nuke script version: v002" in msg for msg in messages)
    ...
    def test_handle_media_loading_plate_only(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.raw_plate_finder, 'find_latest_raw_plate') as mock_find_plate:
            with patch.object(nuke_handler.raw_plate_finder, 'verify_plate_exists') as mock_verify:
                with patch.object(nuke_handler.script_generator, 'create_plate_script') as mock_create_script:
                    with patch.object(nuke_handler.raw_plate_finder, 'get_version_from_path') as mock_get_version:
                        ...
                        assert any("Generated Nuke script with plate: v001" in msg for msg in messages)

    def test_handle_media_loading_undistortion_only(self, nuke_handler, mock_shot):
        """Test media loading with undistortion only."""
        with patch.object(nuke_handler.undistortion_finder, 'find_latest_undistortion') as mock_find_undist:
            with patch.object(nuke_handler.undistortion_finder, 'get_version_from_path') as mock_get_version:
                with patch("nuke_launch_handler.Config.NUKE_UNDISTORTION_MODE", "direct"):
                    ...
                    assert any("Opening undistortion file directly: v001" in msg for msg in messages)
    ...
    def test_handle_media_loading_plate_and_undistortion(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.raw_plate_finder, 'find_latest_raw_plate') as mock_find_plate:
            with patch.object(nuke_handler.raw_plate_finder, 'verify_plate_exists') as mock_verify:
                with patch.object(nuke_handler.undistortion_finder, 'find_latest_undistortion') as mock_find_undist:
                    with patch.object(nuke_handler.script_generator, 'create_loader_script') as mock_create_loader:
                        with patch("nuke_launch_handler.Config.NUKE_USE_LOADER_SCRIPT", True):
                            mock_find_plate.return_value = "/test/plates/TEST_0010_v001.exr"
                            mock_verify.return_value = True
                            mock_find_undist.return_value = Path("/test/undist/TEST_0010_v001.nk")
                            ...
                            with patch.object(nuke_handler.raw_plate_finder, 'get_version_from_path') as mock_plate_ver:
                                with patch.object(nuke_handler.undistortion_finder, 'get_version_from_path') as mock_undist_ver:
                                    mock_plate_ver.return_value = "v001"
                                    mock_undist_ver.return_value = "v002"

                                    options = {"include_raw_plate": True, "include_undistortion": True}
                                    command, messages = nuke_handler._handle_media_loading(
                                        mock_shot, "nuke", options
                                    )
    ...
    def test_get_environment_fixes_with_problematic_plugins(self, nuke_handler):
        ...
        with patch("nuke_launch_handler.Config.NUKE_FIX_OCIO_CRASH", True):
            with patch("nuke_launch_handler.Config.NUKE_SKIP_PROBLEMATIC_PLUGINS", True):
                with patch("nuke_launch_handler.Config.NUKE_PROBLEMATIC_PLUGIN_PATHS", ["/bad/plugin1", "/bad/plugin2"]):
                    fixes = nuke_handler.get_environment_fixes()
    ...
    def test_get_environment_fixes_with_ocio_fallback(self, nuke_handler):
        ...
        with patch("nuke_launch_handler.Config.NUKE_FIX_OCIO_CRASH", True):
            with patch("nuke_launch_handler.Config.NUKE_OCIO_FALLBACK_CONFIG", "/test/ocio/config.ocio"):
    ...
    def test_create_new_workspace_script_with_plate(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.raw_plate_finder, 'find_latest_raw_plate') as mock_find:
            with patch.object(nuke_handler.raw_plate_finder, 'verify_plate_exists') as mock_verify:
                with patch.object(nuke_handler.script_generator, 'create_workspace_plate_script') as mock_create:
    ...
    def test_create_new_workspace_script_empty(self, nuke_handler, mock_shot):
        ...
        with patch.object(nuke_handler.script_generator, 'create_plate_script') as mock_create:
            with patch.object(nuke_handler.script_generator, 'save_workspace_script') as mock_save:
                with patch("builtins.open", create=True) as mock_open:
                    mock_open.return_value.__enter__.return_value.read.return_value = "<nuke_script_content>"
