...
class TestCacheManagerThreading:
    ...
    def test_thumbnail_cache_result_concurrent_completion(self):
        ...
        assert len(successful_completions) == 1, f"Expected 1 successful completion, got {len(successful_completions)}"
    ...
    def test_concurrent_thumbnail_caching(self, cache_manager, test_shot):
        ...
        def cache_thumbnail(thread_id: int):
            ...
            try:
                ...
                shot = Shot(f"show_{thread_id}", "seq_01", f"shot_{thread_id:03d}", f"/path/{thread_id}")
                ...
                with patch('cache_manager.Path.exists', return_value=True), \
                     patch('cache_manager.QImage') as mock_image_class:
                    ...
                    result = manager.cache_thumbnail(Path(f"/test/image_{thread_id}.jpg"), shot.show, shot.sequence, shot.shot)
        ...
        try:
            ...
            with patch('cache_manager.QImage') as mock_image_class:
                ...
                with patch('cache_manager.Path.exists', return_value=True):
                    ...
                    result = manager.cache_thumbnail(Path("/test/large_image.jpg"), shot.show, shot.sequence, shot.shot)
                    
                    # Verify memory tracking
                    assert manager._memory_usage_bytes <= manager._max_memory_bytes or result is None
    ...
    def test_cache_key_generation_thread_safety(self, cache_manager):
        ...
        def generate_cache_keys(thread_id: int):
            ...
            try:
                for i in range(10):
                    shot = Shot(f"show_{thread_id}", f"seq_{i}", f"shot_{i:03d}", f"/path/{thread_id}/{i}")
    ...
    def test_concurrent_cache_retrieval(self, cache_manager, test_shot):
        ...
        def retrieve_cached_thumbnail(thread_id: int):
            ...
            try:
                ...
                with patch('pathlib.Path.exists', return_value=True):
                    for _ in range(5):
                        cached = manager.get_cached_thumbnail(test_shot.show, test_shot.sequence, test_shot.shot)
        ...
        assert len(successful_retrievals) >= 18  # Allow for 1-2 timing misses in threading
