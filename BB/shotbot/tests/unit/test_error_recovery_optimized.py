...
class TestErrorRecovery:
    ...
    def test_network_failure_recovery(self, error_prone_model, qtbot):
        ...
        assert "Network unreachable" in error_spy[0][0]
    ...
    def test_corrupted_cache_recovery(self, error_prone_model):
        ...
        with patch.object(error_prone_model.cache_manager, 'get_cached_shots') as mock_cache:
    ...
    def test_async_loader_exception_handling(self, qtbot):
        ...
        failing_pool = Mock()
        failing_pool.execute_workspace_command.side_effect = RuntimeError("Critical error")
        ...
        assert "Critical error" in error_spy[0][0]
...
workspace /valid/path/seq03/0030"""
        ...
        valid_shots = [shot for shot in error_prone_model.shots if shot.show in ['valid', 'another', 'valid']]
