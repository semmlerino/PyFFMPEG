...
class TestThreeDEScene:
    ...
    def test_scene_creation(self, make_real_3de_file):
        ...
        scene_path = make_real_3de_file("test_show", "seq01", "shot01", "otheruser", "v001")
    ...
    def test_get_thumbnail_path_with_real_files(self, tmp_path, monkeypatch):
        ...
        thumb_file.write_bytes(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00\xff\xd9")
    ...
    def test_get_thumbnail_path_none_found(self, tmp_path):
        ...
        shot_path = tmp_path / "shows" / "test_show" / "shots" / "seq01" / "seq01_shot01"
        shot_path.mkdir(parents=True, exist_ok=True)
...
class TestThreeDESceneModel:
    ...
    def test_refresh_scenes_with_real_files(self, real_cache_manager, tmp_path, monkeypatch):
        ...
        for seq_num in range(2):
            for shot_num in range(2):
                ...
                scene_path = shot_path / "user" / f"artist{seq_num}" / "3de" / "mm-default" / f"{shot_name}_v001.3de"
                scene_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        # Create user shots
        user_shots = [
            Shot("test_show", "seq00", "shot00", str(shows_root / "test_show" / "shots" / "seq00" / "seq00_shot00")),
            Shot("test_show", "seq01", "shot01", str(shows_root / "test_show" / "shots" / "seq01" / "seq01_shot01")),
        ]
    ...
    def test_scenes_property(self, real_cache_manager, make_real_3de_file):
        ...
        model.scenes = [
            ThreeDEScene("show1", "seq01", "shot01", str(scene_path1.parent.parent.parent.parent), 
                        "user1", "FG01", scene_path1),
            ThreeDEScene("show1", "seq01", "shot02", str(scene_path2.parent.parent.parent.parent),
                        "user2", "BG01", scene_path2),
        ]
    ...
    def test_get_scene_by_index(self, real_cache_manager, make_real_3de_file):
        ...
        model.scenes = [
            ThreeDEScene("show1", "seq01", "shot01", str(scene_path.parent.parent.parent.parent),
                        "user1", "FG01", scene_path),
        ]
    ...
    def test_deduplicate_scenes_per_shot(self, real_cache_manager, tmp_path):
        ...
        for i, user in enumerate(["user1", "user2", "user3"]):
            scene_path = base_path / "seq01" / "seq01_shot01" / "user" / user / "3de" / f"scene_v{i+1}.3de"
            scene_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            scenes.append(ThreeDEScene(
                "test", "seq01", "shot01", str(base_path / "seq01" / "seq01_shot01"),
                user, f"FG0{i+1}", scene_path
            ))
        
        # Add a scene for different shot
        other_path = base_path / "seq02" / "seq02_shot01" / "user" / "user4" / "3de" / "scene.3de"
        other_path.parent.mkdir(parents=True, exist_ok=True)
        other_path.write_text("# 3DE scene")
        
        scenes.append(ThreeDEScene(
            "test", "seq02", "shot01", str(base_path / "seq02" / "seq02_shot01"),
            "user4", "BG01", other_path
        ))
    ...
    def test_concurrent_refresh(self, real_cache_manager, make_real_3de_file):
        ...
        user_shots = [
            Shot("show1", "seq01", "shot01", str(scene_path.parent.parent.parent.parent))
        ]
