...
class TestThreeDESceneWorker:
    ...
    def test_do_work_progressive_mode(self, qtbot, sample_shots, sample_scenes):
        ...
        with patch.object(ThreeDESceneFinder, 'estimate_scan_size', return_value=(2, 10)):
            ...
            def mock_progressive(*args, **kwargs):
                for i, scene in enumerate(sample_scenes):
                    yield ([scene], i+1, len(sample_scenes), f"Processing shot {i+1}")
            
            with patch.object(ThreeDESceneFinder, 'find_all_scenes_progressive', return_value=mock_progressive()):
                # Set up signal spies
                started_spy = QSignalSpy(worker.started)
    ...
    def test_do_work_traditional_mode(self, qtbot, sample_shots):
        ...
        with patch.object(ThreeDESceneFinder, 'discover_all_shots_in_show', return_value=mock_discover_result):
    ...
    def test_do_work_error_handling(self, qtbot, sample_shots):
        ...
        with patch.object(ThreeDESceneFinder, 'estimate_scan_size', return_value=(2, 10)):
            with patch.object(ThreeDESceneFinder, 'find_all_scenes_progressive', side_effect=Exception("Test error")):
                # Set up signal spies
    ...
    def test_discover_scenes_traditional_show_discovery(self, sample_shots):
        ...
        with patch.object(ThreeDESceneFinder, 'discover_all_shots_in_show', mock_discover):
            with patch.object(ThreeDESceneFinder, 'find_scenes_for_shot', mock_find_scenes):
                # Run discovery
    ...
    def test_thread_priority_setting(self, sample_shots):
        ...
        with patch('threede_scene_worker.Config.WORKER_THREAD_PRIORITY', 1):
            ...
            with patch.object(ThreeDESceneFinder, 'find_all_scenes_in_shows_efficient', return_value=[]):
                ...
                with patch.object(worker, 'setPriority') as mock_set_priority:
                    ...
                    mock_set_priority.assert_called_once_with(QThread.Priority.HighPriority)
