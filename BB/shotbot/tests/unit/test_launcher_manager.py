...
def test_shot():
    ...
    def _make_shot(show="testshow", sequence="seq01", shot="shot01", workspace_path="/test/workspace"):
        return Shot(
            show=show,
            sequence=sequence,
            shot=shot,
...
class TestLauncherManager:
    ...
    def test_load_and_persist_launchers_from_config(self, launcher_manager, test_launcher, temp_config_dir):
        ...
        test_data = {
            ...
            "launchers": {
                "launcher1": {
                    ...
                    "validation": {"check_executable": True, "required_files": [], "forbidden_patterns": []},
    ...
    def test_delete_launcher_with_real_persistence(self, launcher_manager):
        ...
        assert len(changed_signals) >= 1  # Delete (create happened before signal connection)
...
class TestLauncherExecution:
    ...
    def test_process_limit_enforcement(self, mock_qtimer, mock_config_class, mock_pool):
        ...
        manager.validation_error.emit = Mock(side_effect=lambda field, error: validation_errors.append((field, error)))
        
        result = manager.execute_launcher("test")
        
        assert result is False
        assert len(validation_errors) > 0
        assert any("Maximum concurrent processes" in error for field, error in validation_errors)
...
class TestLauncherWorker:
    ...
    def test_launcher_worker_signals(self):
        ...
        signal_names = [name for name, obj in worker_members if 'Signal' in str(type(obj))]
...
class TestThreadSafety:
    ...
    @patch('launcher_manager.LauncherConfig')
    @patch('launcher_manager.QTimer')
    def test_concurrent_launcher_creation(self, mock_qtimer, mock_config_class, mock_pool):
...
class TestCommandValidation:
    ...
    def test_validate_launcher_paths_real_validation(self, launcher_manager, test_shot, tmp_path):
...
class TestConcurrentExecution:
    ...
    def test_concurrent_launcher_execution_real_workers(self, temp_config_dir):
        ...
        """
        import pytest
        pytest.skip("Qt threading with signal emission not reliable in test environment")
        ...
        with patch('launcher_manager.subprocess.Popen') as mock_popen:
            ...
            assert len(started_signals) >= num_launchers // 2  # At least half should start
    ...
    def test_process_limit_enforcement_under_concurrent_load(self, temp_config_dir):
        ...
        if len(validation_errors) > 0:
            error_messages = [error for field, error in validation_errors]
            assert all("Maximum concurrent processes" in error for error in error_messages)
    ...
    def test_worker_execution_with_real_qt_signals(self, temp_config_dir):
        ...
        """
        import pytest
        pytest.skip("Qt signal emission across threads not reliable in test environment")
        launcher_manager = create_real_launcher_manager(temp_config_dir)
        ...
        worker.command_finished.connect(
            lambda lid, success, code: log_signal("command_finished", lid, success, code)
        )
        ...
        with patch('launcher_manager.subprocess.Popen') as mock_popen:
            ...
            mock_process.wait.side_effect = [subprocess.TimeoutExpired("test", 1)] * 2 + [0]
            mock_popen.return_value = mock_process
        ...
        if start_time and finish_time:
            assert start_time < finish_time, "Started signal should come before finished"
...
class TestThreadSafetyAdvanced:
    ...
    def test_process_key_uniqueness_under_concurrent_load(self, temp_config_dir):
        ...
        assert unique_keys == total_keys, f"Found {total_keys - unique_keys} duplicate keys!"
        ...
        for key in generated_keys[:10]:  # Check first 10 keys
            ...
            assert key.startswith("launcher_"), f"Should start with launcher prefix: {key}"
    ...
    def test_rlock_protection_verification(self, temp_config_dir):
        ...
        def process_operations(thread_id):
            ...
            for i in range(20):
                ...
                if i % 5 == 0:
                    with launcher_manager._process_lock:
                        keys_to_remove = [k for k in launcher_manager._active_processes.keys() 
        ...
        assert all(count >= 0 for count in counts), "Process counts should never be negative"
    ...
    def test_cleanup_coordination_with_real_locking(self, temp_config_dir):
        ...
        def concurrent_cleanup(thread_id):
            ...
            log_cleanup(thread_id, "START_PERIODIC", len(launcher_manager._active_processes))
            launcher_manager._periodic_cleanup()
            log_cleanup(thread_id, "END_PERIODIC", len(launcher_manager._active_processes))
            
            # Try finished process cleanup
            log_cleanup(thread_id, "START_FINISHED", len(launcher_manager._active_processes))
            launcher_manager._cleanup_finished_processes()
            log_cleanup(thread_id, "END_FINISHED", len(launcher_manager._active_processes))
        ...
        assert final_count == 0, f"Expected 0 processes after cleanup, got {final_count}"
        ...
        assert len(start_operations) == len(end_operations), "All cleanup operations should complete"
...
class TestLauncherWorkerLifecycle:
    ...
    def test_launcher_worker_state_transitions(self):
        ...
        with patch('launcher_manager.subprocess.Popen') as mock_popen:
            ...
            mock_process.poll.return_value = None
            mock_process.wait.side_effect = [subprocess.TimeoutExpired("test", 1)] * 3 + [0]
            mock_popen.return_value = mock_process
    ...
    def test_launcher_worker_signal_emission_order(self):
        ...
        worker.worker_started.connect(
            lambda: log_signal("worker_started")
        )
        worker.worker_stopped.connect(
            lambda: log_signal("worker_stopped")
        )
        ...
        started_indices = [i for i, name in enumerate(signal_names) if name == "command_started"]
        finished_indices = [i for i, name in enumerate(signal_names) if name == "command_finished"]
        
        if started_indices and finished_indices:
            assert min(started_indices) < max(finished_indices), "Started should come before finished"
        
        # Verify signal timing consistency
        assert all(signal_times[i] <= signal_times[i+1] for i in range(len(signal_times)-1)), \
    ...
    def test_multiple_workers_concurrent_execution(self, temp_config_dir):
        ...
        for i, worker in enumerate(workers):
            ...
            worker.command_finished.connect(
                lambda lid, success, code, wid=worker_id: collect_signal(wid, "finished", lid, success, code)
            )
        ...
        started_workers = set(entry[1] for entry in all_signals if entry[2] == "started")
        assert len(started_workers) >= num_workers // 2, "Most workers should have started"
...
class TestSignalThreadSafety:
    ...
    def test_cross_thread_signal_emission_integrity(self, temp_config_dir):
        ...
        launcher_manager.execution_finished.connect(
            lambda lid, success: log_signal_with_thread("execution_finished", lid, success)
        )
