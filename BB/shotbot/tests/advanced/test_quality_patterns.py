...
class ShotModelBuilder(Builder):
    ...
    def build(self) -> "ShotModel":
        ...
        model = ShotModel(load_cache=False)  # Start with clean state
...
class CacheBuilder(Builder):
    ...
    def build(self) -> "CacheManager":
        ...
        for key, (value, ttl) in self._entries.items():
            # Note: CacheManager doesn't have a generic set method
            # Store entries for test purposes in a test attribute
            if not hasattr(cache, "_test_entries"):
                cache._test_entries = {}
            cache._test_entries[key] = (value, ttl)
...
class TestWithFactories:
    ...
    def test_cache_with_builder(self):
        ...
        # Verify state (using test attribute since cache doesn't have generic get)
        assert hasattr(cache, "_test_entries")
        assert cache._test_entries["key1"][0] == "value1"
        assert cache._test_entries["key2"][0] == "value2"
        # Expired entry should still exist in test entries, but would be None in real usage
        assert cache._test_entries["expired"][0] == "old_value"
...
if __name__ == "__main__":
    ...
    print(f"Built model with {len(model.shots)} shots")
