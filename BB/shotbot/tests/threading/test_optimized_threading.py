...
from shot_model_optimized import OptimizedShotModel, AsyncShotLoader
from shot_model import Shot, RefreshResult
...
workspace /shows/TEST/seq02/0030"""
    ...
    def test_no_signal_emission_after_stop(self):
        ...
        loader.load_failed.connect(lambda x: signals_received.append("failed"))
    ...
    def test_concurrent_stop_during_execution(self):
...
class TestOptimizedShotModelThreadSafety:
    ...
    def test_concurrent_background_refresh(self):
        ...
        with patch.object(AsyncShotLoader, 'start', mock_start):
            ...
            def attempt_refresh():
                self.model._start_background_refresh()
    ...
    def test_cleanup_during_active_loading(self):
        ...
        with patch.object(AsyncShotLoader, 'run') as mock_run:
            ...
            assert self.model._async_loader is None, "Loader should be cleaned up"
    
    def test_race_condition_protection_in_refresh(self):
        ...
        with patch.object(Shot, 'refresh_shots', count_refresh):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(self.model.refresh_shots)
                    for _ in range(20)
                ]
...
class TestProcessPoolManagerSingleton:
    ...
    def test_session_pool_creation_race(self):
        ...
        with patch.object(PersistentBashSession, '__init__', counting_init):
            ...
            def get_session():
                return manager._get_bash_session("test_type")
            ...
            expected = manager._sessions_per_type
            assert creation_count[0] == expected, f"Should create exactly {expected} sessions"
...
class TestDeadlockDetection:
    ...
    def test_no_deadlock_in_cleanup(self):
    ...
    def test_signal_emission_no_deadlock(self):
...
class TestStressAndPerformance:
    ...
    def test_stress_concurrent_operations(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(perform_operation, i % 4)
                for i in range(100)
            ]
