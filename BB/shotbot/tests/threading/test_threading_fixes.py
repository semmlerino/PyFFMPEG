...
class TestWorkerStateTransitions:
    ...
    def test_concurrent_state_transitions(self, qtbot):
        ...
        assert (
            WorkerState.STOPPING in valid_targets or WorkerState.ERROR in valid_targets
        )
        ...
        assert invalid_targets.intersection(expected_invalid), (
            "Expected invalid transitions not found"
        )
    ...
    def test_state_machine_integrity(self, qtbot):
        ...
        for i, worker in enumerate(workers):
            thread = threading.Thread(target=monitor_state_changes, args=(i, worker))
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                ...
                if len(results) == 0 and len(errors) > 0:
                    ...
                    assert len(results) >= 1, (
                        f"No sessions executed successfully. Results: {results}, Errors: {errors}"
                    )
...
class TestFutureBasedSynchronization:
    ...
    def test_concurrent_cache_requests(self, cache_manager, tmp_path):
        ...
        assert len(request_results) == 10, (
            f"Expected 10 results, got {len(request_results)}"
        )
        ...
        assert len(successful_requests) >= 5, (
            f"Too many failures: {10 - len(successful_requests)} failures out of 10"
        )
...
class TestComprehensiveThreadingStress:
    ...
    def test_full_lifecycle_under_load(self, qtbot, tmp_path, mock_subprocess):
        ...
        assert len(operation_results) >= 6, (
            f"Expected at least 6 operation results, got {len(operation_results)}"
        )
        ...
        for op_type, results in results_by_type.items():
            ...
            assert len(successful) >= 1, (
                f"No successful {op_type} operations: {results}"
            )
        ...
        assert all(not f.running() for f in futures), (
            "Some operations are still running (possible deadlock)"
        )
