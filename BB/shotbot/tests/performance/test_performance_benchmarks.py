...
from cache.thumbnail_processor_optimized import OptimizedThumbnailProcessor
...
class MockProcessPoolManager:
    ...
    def execute_workspace_command(self, command: str, cache_ttl: int = 30, **kwargs) -> str:
...
class TestProcessPoolManagerPerformance:
    ...
    def test_subprocess_startup_performance(self, test_commands, qtbot):
        ...
        # Verify improvement
        print("\n" + result.report())
        assert result.improvement_percentage >= 50, f"Expected ≥50% improvement, got {result.improvement_percentage:.1f}%"
    ...
    def test_cache_effectiveness(self, test_commands, qtbot):
        ...
        assert cache_hit_rate >= 0.5, f"Expected ≥50% cache hit rate, got {cache_hit_rate:.1%}"
        assert result.improvement_percentage >= 40, f"Expected ≥10% improvement, got {result.improvement_percentage:.1f}%"
    ...
    def test_parallel_execution_performance(self, workspace_commands, qtbot):
        ...
        print("\n" + result.report())
        assert result.improvement_percentage >= 30, f"Expected ≥30% improvement, got {result.improvement_percentage:.1f}%"
...
class TestThumbnailProcessorPerformance:
    ...
    def test_sequential_vs_parallel_processing(self, test_images, tmp_path):
        ...
        optimized_processor = OptimizedThumbnailProcessor(max_workers=4)
        cache_dir = tmp_path / "cache_optimized"
        cache_dir.mkdir()
        
        # Prepare batch requests
        requests = [
            (img_path, cache_dir / f"thumb_{idx}.jpg")
            for idx, img_path in enumerate(test_images)
        ]
        
        start = time.perf_counter()
        batch_result = optimized_processor.process_batch(requests)
        result.optimized_time = (time.perf_counter() - start) * 1000
        
        # Verify all thumbnails were created
        assert len(batch_result.successful) == len(test_images)
        ...
        assert result.improvement_percentage >= -30, f"Parallel processing too slow ({result.improvement_percentage:.1f}% degradation exceeds -30% threshold)"
        print(f"  Parallel vs Sequential: {result.improvement_percentage:.1f}% ({'improvement' if result.improvement_percentage > 0 else 'overhead'})")
    ...
    def test_smart_backend_selection(self, test_images, tmp_path):
        ...
        # Create processor with smart backend selection
        processor = OptimizedThumbnailProcessor()
        ...
        assert metrics['average_time_ms'] < 100, f"Expected <100ms per image, got {metrics['average_time_ms']:.2f}ms"
    ...
    def test_memory_efficiency(self, test_images, tmp_path):
        ...
        # Measure memory with optimized processor
        optimized_processor = OptimizedThumbnailProcessor(max_workers=4)
        initial_memory = process.memory_info().rss / 1024 / 1024
        
        requests = [
            (img_path, tmp_path / f"opt_{idx}.jpg")
            for idx, img_path in enumerate(test_images * 2)
        ]
        optimized_processor.process_batch(requests)
        ...
        print(f"  Memory saved: {original_memory_increase - optimized_memory_increase:.1f}MB")
...
class TestCachePerformance:
    ...
    def test_cache_ttl_extension(self, tmp_path, sample_shots):
        ...
        assert result.optimized_time < 10, f"Cache access too slow: {result.optimized_time:.2f}ms"
    ...
    def test_memory_cache_performance(self, tmp_path):
        ...
        assert cache_hit_time < cache_miss_time * 0.1, "Memory cache should be >10x faster"
