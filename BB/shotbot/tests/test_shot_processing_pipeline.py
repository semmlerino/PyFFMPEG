...
def parse_ws_sg_output(output: str) -> List[Shot]:
    ...
    for line in output.strip().split('\n'):
        ...
        if len(path_parts) >= 5 and path_parts[1] == 'shows' and path_parts[3] == 'shots':
            show = path_parts[2]
...
def test_ws_sg_parsing():
    ...
    assert len(shots) == expected_count, f"Expected {expected_count} shots, got {len(shots)}"
    ...
    for idx, show, sequence, shot in test_cases:
        s = shots[idx]
        assert s.show == show and s.sequence == sequence and s.shot == shot, f"Shot {idx}: expected {show}/{sequence}/{shot}, got {s.show}/{s.sequence}/{s.shot}"
...
def test_targeted_shot_finder():
    ...
    try:
        ...
        assert target_shows == expected_shows, f"Expected shows {expected_shows}, got {target_shows}"
    ...
    try:
        ...
        test_paths = [
            (Path("/shows/gator/shots/012_DC/012_DC_1000/user/ryan/mm/3de/test.3de"), 
             "gator", "012_DC", "1000"),
            (Path("/shows/jack_ryan/shots/DB_271/DB_271_1760/user/john/scenes/test.3de"),
             "jack_ryan", "DB_271", "1760"),
            (Path("/shows/broken_eggs/shots/BRX_166/BRX_166_0010/publish/mm/test.3de"),
        ...
        for file_path, expected_show, expected_seq, expected_shot in test_paths:
            ...
            assert show == expected_show and sequence == expected_seq and shot == expected_shot, f"{file_path.name}: expected {expected_show}/{expected_seq}/{expected_shot}, got {show}/{sequence}/{shot}"
...
def test_integration():
    ...
    try:
        ...
        assert len(deduplicated) == 1, f"Deduplication failed: expected 1 scene, got {len(deduplicated)}"
