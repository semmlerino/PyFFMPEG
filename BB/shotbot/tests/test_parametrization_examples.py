...
class TestParametrizationPatterns:
    ...
    @pytest.mark.parametrize("data_size,expected_time", [
        ...
        pytest.param(10000, 10.0, marks=pytest.mark.slow, id="huge_dataset_very_slow"),
    ])
    def test_performance_aware_parametrization(self, data_size: int, expected_time: float):
        """Parametrization with performance marks for slow tests."""
        # Implementation would go here
        pass

    # 3. Complex parametrization replacing manual loops
    @pytest.mark.parametrize("show,sequence,shot", [
        ...
        pytest.param("complex_show", "long_seq_name", "9999", 
                    marks=pytest.mark.slow, id="complex_naming_slow"),
    ])
    ...
    @pytest.mark.parametrize("app_name,startup_time", [
        # Fast applications
        ...
        pytest.param("houdini", 8.0, marks=pytest.mark.slow, id="houdini_very_slow_startup"),
    ])
    ...
    @pytest.mark.parametrize("file_count,processing_time", [
        # Normal load tests
        *[pytest.param(count, time, id=f"files_{count}") 
          for count, time in [(10, 0.5), (50, 1.0), (100, 2.0)]],
        # Heavy load tests (all marked slow)
        *[pytest.param(count, time, marks=pytest.mark.slow, id=f"heavy_files_{count}")
    ...
    @pytest.mark.parametrize("show,sequence,shot", [
        ...
        *[pytest.param(s, seq, shot, id=f"{s}_{seq}_{shot}") 
          for s, seq, shot in generate_test_combinations(None)[:4]],  # Limit for example
        # Add special cases with custom marks
        pytest.param("edge_case_show", "edge_seq", "9999", 
                    marks=pytest.mark.slow, id="edge_case_slow"),
    ])
...
@pytest.fixture(params=[
    ...
    }, marks=pytest.mark.slow, id="advanced_config_slow"),
])
