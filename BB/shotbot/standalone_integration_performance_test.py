...
class MockVFXStructure:
    ...
    def _create_mock_file(self, filename: str) -> Mock:
        ...
        mock_file.is_file.return_value = filename.endswith(('.jpg', '.exr', '.3de', '.nk'))
...
def test_shot_list_refresh_performance():
    ...
    for seq_idx in range(3):
        ...
        for shot_idx in range(33):  # ~100 shots total
            ...
            mock_shots_output.append(f"project_00\t{sequence}\t{shot_name}\t{workspace}")
    ...
    with patch('subprocess.run') as mock_run:
        ...
        with patch.object(Path, 'exists', side_effect=mock_vfx.exists):
            ...
            cache_speedup = first_refresh_time / second_refresh_time if second_refresh_time > 0 else 1.0
            fs_reduction_percent = (1 - (additional_fs_accesses / initial_fs_accesses)) * 100 if initial_fs_accesses > 0 else 0
            
            print("\nPERFORMANCE RESULTS:")
            print(f"First refresh:  {first_refresh_time:.3f}s ({initial_fs_accesses} FS accesses)")
            print(f"Second refresh: {second_refresh_time:.3f}s ({additional_fs_accesses} additional FS accesses)")
    ...
    with patch.object(Path, 'exists', side_effect=mock_vfx.exists):
        with patch.object(Path, 'iterdir', side_effect=mock_vfx.iterdir):
            ...
            for workspace in shot_workspaces:
                try:
                    scenes = ThreeDESceneFinder.find_threede_scenes(workspace, {"current_user"})
            ...
            print(f"   Avg time per shot: {(discovery_time * 1000) / len(shot_workspaces):.1f}ms")
            print(f"   Avg FS accesses per shot: {total_fs_accesses / len(shot_workspaces):.1f}")
            ...
            print("\nPERFORMANCE VALIDATION:")
            if time_passed:
                print(f"✓ Time per shot acceptable: {time_per_shot_ms:.1f}ms <= {max_time_per_shot}ms")
            else:
                print(f"! Time per shot high: {time_per_shot_ms:.1f}ms > {max_time_per_shot}ms")
            
            if fs_passed:
                print(f"✓ FS accesses per shot acceptable: {fs_per_shot:.1f} <= {max_fs_per_shot}")
            else:
                print(f"! FS accesses per shot high: {fs_per_shot:.1f} > {max_fs_per_shot}")
...
def test_raw_plate_finding_performance():
    ...
    with patch.object(Path, 'exists', side_effect=mock_vfx.exists):
        with patch.object(Path, 'iterdir', side_effect=mock_vfx.iterdir):
            ...
            print(f"   FS accesses: {total_fs_accesses}")
            print(f"   Avg time per shot: {(finding_time * 1000) / len(test_shots):.1f}ms")
            ...
            if success_passed:
                print(f"✓ Success rate acceptable: {success_rate:.1f}% >= {min_success_rate}%")
            else:
...
def test_end_to_end_workflow_performance():
    ...
    for step_name, step_time, operations in workflow_steps:
        ...
        print(f"  {step_name}: {step_time:.3f}s ({operations} operations, {ops_per_sec:.1f} ops/sec)")
    ...
    print("\nWORKFLOW VALIDATION:")
    if time_passed:
        print(f"✓ Total time acceptable: {total_workflow_time:.3f}s <= {max_total_time}s")
    else:
