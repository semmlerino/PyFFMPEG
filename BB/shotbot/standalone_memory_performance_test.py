...
def test_cache_memory_management():
    ...
    print(f"✓ Average access time: {(access_time * 1000) / (num_entries // 10):.2f}ms per entry")
    ...
    if memory_increase < max_acceptable_increase:
        print(f"✓ Memory increase acceptable: {memory_increase:.1f}MB < {max_acceptable_increase}MB")
        memory_passed = True
    else:
        print(f"! Memory increase high: {memory_increase:.1f}MB >= {max_acceptable_increase}MB")
        memory_passed = False
    ...
    if recovery_percentage >= min_recovery_percentage:
        print(f"✓ Memory recovery good: {recovery_percentage:.1f}% >= {min_recovery_percentage}%")
        recovery_passed = True
    else:
        print(f"! Memory recovery low: {recovery_percentage:.1f}% < {min_recovery_percentage}%")
...
def test_garbage_collection_efficiency():
    ...
    print(f"✓ Memory after creation: {after_creation_memory:.1f}MB (+{memory_used:.1f}MB)")
    ...
    object_recovery_percentage = (objects_recovered / objects_created) * 100 if objects_created > 0 else 0
    memory_recovery_percentage = (memory_recovered / memory_used) * 100 if memory_used > 0 else 0
    ...
    if object_gc_passed:
        print(f"✓ Object GC efficient: {object_recovery_percentage:.1f}% >= {min_object_recovery}%")
    else:
        print(f"! Object GC inefficient: {object_recovery_percentage:.1f}% < {min_object_recovery}%")
        
    if memory_gc_passed:
        print(f"✓ Memory GC efficient: {memory_recovery_percentage:.1f}% >= {min_memory_recovery}%")
    else:
        print(f"! Memory GC inefficient: {memory_recovery_percentage:.1f}% < {min_memory_recovery}%")
