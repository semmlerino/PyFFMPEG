...
def debug_parser_matching(lines):
    ...
    for i, line in enumerate(lines[:10]):  # Test first 10 lines
        ...
        if match:
            ...
            if result:
                print(f"  ✅ Parse result: show={result.show}, seq={result.sequence}, shot={result.shot}")
                successful_parses += 1
            else:
            ...
            if "workspace" in line:
                ...
                if Config.SHOWS_ROOT not in line:
                    print(f"     💡 Line doesn't contain expected SHOWS_ROOT: {Config.SHOWS_ROOT}")
    ...
    print(f"Summary: {successful_parses} successful parses, {failed_parses} failed parses")
