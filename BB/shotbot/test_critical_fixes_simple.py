...
def test_previous_shots_cleanup():
    ...
    total_tests += 1
    if "if hasattr(self, 'previous_shots_model') and self.previous_shots_model:" in content:
...
def test_json_error_handling():
    ...
    total_tests += 1
    if "except (IOError, OSError) as e:" in content or "except OSError as e:" in content:
...
def test_config_shows_root():
    ...
    total_tests += 1
    if 'SHOWS_ROOT: str = os.environ.get("SHOWS_ROOT", "/shows")' in content or \
       'SHOWS_ROOT = os.environ.get("SHOWS_ROOT", "/shows")' in content:
    ...
    total_tests += 1
    if 'SHOW_ROOT_PATHS = [SHOWS_ROOT]' in content or \
       'SHOW_ROOT_PATHS: list[str] = [SHOWS_ROOT]' in content:
    ...
    for shows_root in test_values:
        ...
        try:
            ...
            if config.Config.SHOWS_ROOT == shows_root:
                ...
                print(f"  ✗ Config.SHOWS_ROOT is {config.Config.SHOWS_ROOT}, expected {shows_root}")
