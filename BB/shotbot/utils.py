...
class PathUtils:
    ...
    def find_user_workspace_jpeg_thumbnail(
        ...
        try:
            ...
            for user_path in user_dir.iterdir():
                ...
                # Check for Nuke outputs path
                nuke_outputs = user_path / "mm" / "nuke" / "outputs" / "mm-default" / "scene"
                if not nuke_outputs.exists():
                    continue

                # Discover plate directories using dynamic discovery
                            ...
                            for resolution_dir in potential_jpeg_path.iterdir():
                                ...
                                if jpeg_file and jpeg_file.suffix.lower() in ['.jpg', '.jpeg']:
                                    logger.info(
                                        ...
                                        f"(user: {user_path.name}, plate: {plate_name}, version: {latest_version})"
                                    )
                                    return jpeg_file
                        except (OSError, PermissionError):
                            continue
    ...
    def find_shot_thumbnail(
        ...
        1. Editorial directory thumbnails (publish/editorial/cutref/)
        2. Turnover plate thumbnails (publish/turnover/plate/)
        3. Undistorted JPEG plates from mm/default structure (publish/mm/default/)
        4. User workspace Nuke-generated JPEGs (user/{any_user}/mm/nuke/outputs/)
        5. Any image file containing '1001' in publish folder
        ...
        # Fourth fallback: user workspace Nuke-generated JPEGs (often higher quality)
        ...
        # Fifth fallback: any image with 1001 in publish folder
