...
class PathUtils:
    ...
    @staticmethod
    def build_thumbnail_path(shows_root: str, show: str, sequence: str, shot: str) -> Path:
    ...
    def _cleanup_path_cache():
        ...
        current_time = time.time()
        expired_keys = [
            key for key, (_, timestamp) in _path_cache.items()
            if current_time - timestamp >= _PATH_CACHE_TTL
        ]
...
class VersionUtils:
    ...
    def _cleanup_version_cache():
        ...
        current_time = time.time()
        expired_keys = [
            key for key, (_, timestamp) in VersionUtils._version_cache.items()
            if current_time - timestamp >= _PATH_CACHE_TTL
        ]
        for key in expired_keys:
    ...
    def get_latest_version(base_path: Union[str, Path]) -> Optional[str]:
        ...
        latest_version = version_dirs[-1][1]  # Get the version string from the last (highest) version
...
class FileUtils:
    ...
    @staticmethod
    def validate_file_size(file_path: Union[str, Path], max_size_mb: int = None) -> bool:
        ...
        try:
            ...
            if size_mb > max_size_mb:
                logger.warning(f"File too large ({size_mb:.1f}MB > {max_size_mb}MB): {path_obj}")
                return False
...
class ImageUtils:
    ...
    def validate_image_dimensions(
        ...
        if width > max_dimension or height > max_dimension:
            logger.warning(f"Image dimensions too large ({width}x{height} > {max_dimension})")
            return False
    ...
    @staticmethod
    def get_safe_dimensions_for_thumbnail(max_size: int = None) -> Tuple[int, int]:
...
class ValidationUtils:
    ...
    @staticmethod
    def validate_not_empty(*values: Union[str, None], names: Optional[List[str]] = None) -> bool:
    ...
    def get_current_username() -> str:
        ...
        logger.debug(f"No username found in environment, using default: {Config.DEFAULT_USERNAME}")
