...
def test_process_pool_manager_fix():
    ...
    assert hasattr(manager._session_condition, 'notify_all'), "Missing notify_all method"
...
def test_base_model_functionality():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for model, name in [(standard, "ShotModel"), (optimized, "OptimizedShotModel")]:
            assert hasattr(model, '_parse_ws_output'), f"{name} missing _parse_ws_output"
            assert hasattr(model, 'get_shots'), f"{name} missing get_shots"
            assert hasattr(model, 'get_performance_metrics'), f"{name} missing get_performance_metrics"
...
def test_performance_metrics():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for model_class, name in [(ShotModel, "ShotModel"), 
