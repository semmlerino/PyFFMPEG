...
def test_shows_root() -> bool:
    ...
    assert Config.SHOWS_ROOT == '/tmp/mock_vfx', f"Expected /tmp/mock_vfx, got {Config.SHOWS_ROOT}"
    assert Config.SHOW_ROOT_PATHS == ['/tmp/mock_vfx'], f"Expected ['/tmp/mock_vfx'], got {Config.SHOW_ROOT_PATHS}"
            ...
            for i, line in enumerate(lines):
                if 'def find_user_shots_parallel(' in line:
                    ...
                    if 'Path("/shows")' in func_lines:
                        ...
                        return False
                    if 'Path | None = None' in func_lines or 'Path | None = None' in func_lines:
    ...
    targeted_pattern = re.compile(rf"{shows_root_escaped}/([^/]+)/shots/([^/]+)/([^/]+)/")
    ...
    prev_pattern = re.compile(rf"{shows_root_escaped}/([^/]+)/shots/([^/]+)/\2_([^/]+)/")
    ...
    if match2:
        ...
        print("   ✓ Previous shots pattern correctly doesn't match (needs exact seq_ prefix)")
