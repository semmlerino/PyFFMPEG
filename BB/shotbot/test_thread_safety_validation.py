...
class ThreadSafetyValidationTests(unittest.TestCase):
    ...
    def test_threadsafe_progress_tracker(self):
        ...
        tracker = ThreadSafeProgressTracker(progress_callback=mock_callback, update_interval=5)
        
        # Simulate concurrent workers
        def simulate_worker(worker_id: str, max_files: int):
            for i in range(max_files):
                tracker.report_progress(worker_id, i + 1, f"Worker {worker_id} processing file {i+1}")
        ...
        for i in range(num_workers):
            worker = threading.Thread(target=simulate_worker, args=(f"worker_{i}", files_per_worker))
    ...
    def test_cancellation_event_system(self):
        ...
        cancel_event.add_cleanup_callback(lambda: cleanup_calls.append("second_cleanup"))
        
        # Test initial state
        self.assertFalse(cancel_event.is_cancelled(), "Should not be cancelled initially")
        ...
        self.assertTrue(cancel_event.is_cancelled(), "Should be cancelled after cancel() call")
        self.assertEqual(len(cleanup_calls), 2, "Should have called both cleanup callbacks")
    ...
    def test_threadpool_manager(self):
        ...
        self.assertGreaterEqual(cancelled_count, 1, "At least one task should be cancelled")
    ...
    def test_performance_baseline(self):
        ...
        tracker = ThreadSafeProgressTracker(progress_callback=progress_callback, update_interval=10)
        ...
        def parallel_worker(worker_id: str, count: int):
            for i in range(count):
                tracker.report_progress(worker_id, i + 1, f"Worker {worker_id} file {i}")
    ...
    def test_parallel_scanner_integration(self):
        ...
        try:
            ...
            self.assertTrue(hasattr(OptimizedThreeDESceneFinder, 'find_all_3de_files_in_show_parallel'))
