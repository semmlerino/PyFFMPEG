...
DEBUG_ENABLED = os.environ.get("SHOTBOT_THREAD_MONITOR_DEBUG", "").lower() in ("1", "true", "yes")
DEFAULT_INTERVAL = float(os.environ.get("SHOTBOT_THREAD_MONITOR_INTERVAL", "5.0"))
MONITORING_DISABLED = os.environ.get("SHOTBOT_THREAD_MONITOR_DISABLE", "").lower() in ("1", "true", "yes")
...
class MetricsCollector:
    ...
    def get_metrics(self, 
                   ...
                   since: Optional[float] = None) -> List[ThreadMetric]:
...
class QtThreadMonitor:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self._monitored_threads = {ref for ref in self._monitored_threads if ref() is not None}
            ...
            for thread_ref in self._monitored_threads:
                ...
                thread_info = {
                    ...
                    "priority": thread.priority().name if hasattr(thread.priority(), 'name') else str(thread.priority()),
                }
                ...
                if last_response > 0:
                    ...
                    thread_info["responsive"] = time_since_response < self.max_heartbeat_delay
        ...
        if health_data["summary"]["unresponsive_threads"] > 0:
            health_data["status"] = "warning" if health_data["summary"]["unresponsive_threads"] < health_data["summary"]["total_threads"] / 2 else "critical"
...
class WorkerThreadMonitor:
    ...
    def register_worker(self, worker: 'ThreadSafeWorker') -> None:
        ...
        with self._lock:
            ...
            if hasattr(worker, 'worker_started'):
                worker.worker_started.connect(lambda: self._on_worker_state_change(worker, WorkerState.RUNNING))
            if hasattr(worker, 'worker_stopping'):
                worker.worker_stopping.connect(lambda: self._on_worker_state_change(worker, WorkerState.STOPPING))
            if hasattr(worker, 'worker_stopped'):
                worker.worker_stopped.connect(lambda: self._on_worker_state_change(worker, WorkerState.STOPPED))
            if hasattr(worker, 'worker_error'):
                worker.worker_error.connect(lambda msg: self._on_worker_error(worker, msg))
            
            logger.debug(f"Registered ThreadSafeWorker for monitoring: {worker}")
    
    def _on_worker_state_change(self, worker: 'ThreadSafeWorker', new_state: WorkerState) -> None:
        ...
        if new_state == WorkerState.STOPPED:
            ...
            if start_time:
                ...
                duration_metric = ThreadMetric(
                    ...
                    value=duration,
                    metadata={"final_state": new_state.value if hasattr(new_state, 'value') else str(new_state)}
                )
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self._monitored_workers = {ref for ref in self._monitored_workers if ref() is not None}
            ...
            for worker_ref in self._monitored_workers:
                ...
                worker_info = {
                    ...
                    "is_zombie": getattr(worker, '_zombie', False) if hasattr(worker, '_zombie') else False
                }
                ...
                if current_state == WorkerState.RUNNING and worker_info["lifetime"] > 3600:  # 1 hour
                    health_data["issues"].append({
        ...
        # Determine overall status
        if health_data["summary"]["error_workers"] > 0 or health_data["summary"]["zombie_workers"] > 0:
            health_data["status"] = "warning"
        
        if health_data["summary"]["error_workers"] > health_data["summary"]["total_workers"] / 2:
...
class ProcessPoolMonitor:
    ...
    def record_process_end(self, process_id: int, return_code: int, duration: float) -> None:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self._monitored_pools = {ref for ref in self._monitored_pools if ref() is not None}
        ...
        failure_rate = 0
        total_processes = health_data["summary"]["processes_completed"] + health_data["summary"]["processes_failed"]
...
class ThreadHealthMonitor(QObject):
    ...
    def __init__(self):
        ...
        if ThreadHealthMonitor._instance is not None:
            raise RuntimeError("ThreadHealthMonitor is a singleton. Use get_instance().")
    ...
    def start_monitoring(self) -> None:
        ...
        logger.info(f"Started thread health monitoring (interval: {self.monitoring_interval}s)")
    ...
    def _health_check_impl(self) -> None:
        ...
        health_score = self._calculate_health_score(qt_health, worker_health, process_health)
        ...
        if self.diagnostic_mode:
            logger.debug(f"Health check completed: score={health_score}, duration={health_data['check_duration']:.3f}s")
    
    def _calculate_health_score(self, qt_health: Dict, worker_health: Dict, process_health: Dict) -> int:
    ...
    def _check_for_alerts(self, health_data: Dict) -> None:
        ...
        if health_score < 50:
            alert = HealthAlert(
                ...
                severity=HealthStatus.CRITICAL if health_score < 30 else HealthStatus.WARNING,
                component="Overall",
        ...
        for component_name, component_data in health_data["components"].items():
            ...
            for issue in issues:
                severity = HealthStatus.CRITICAL if issue["type"] in ["zombie_worker", "high_failure_rate"] else HealthStatus.WARNING
                alert = HealthAlert(
                    timestamp=current_time,
                    severity=severity,
    ...
    def get_health_report(self, format: str = "dict") -> Union[Dict, str]:
        ...
        if not self._health_history:
            ...
            if len(self._health_history) >= 2:
                ...
                report["trend"] = {
                    "direction": "improving" if current_score > prev_score else "degrading" if current_score < prev_score else "stable",
    ...
    def get_detailed_report(self, format: str = "dict") -> Union[Dict, str]:
        ...
        report["detailed_metrics"] = {
            ...
            "recent_metrics": self.metrics_collector.get_metrics(since=time.time() - 300),  # Last 5 minutes
            "monitoring_config": {
