...
USE_PYTHON_FILE_DISCOVERY = (
    os.environ.get("SHOTBOT_USE_PYTHON_DISCOVERY", "true").lower() == "true"
)
FALLBACK_TO_SUBPROCESS = (
    os.environ.get("SHOTBOT_FALLBACK_TO_SUBPROCESS", "true").lower() == "true"
)
...
class ThreeDESceneFinder:
            ...
            if has_user_dir:
                for user_path in user_dir.iterdir():
                    ...
                    try:
                        ...
                        threede_files = []
                        use_find_command = (
                            os.environ.get("SHOTBOT_USE_FIND", "true").lower() == "true"
                        )
                            ...
                            try:
                                ...
                                result = subprocess.run(
                                    [
                                        ...
                                        str(user_path),
                                        "-maxdepth",
                                        "15",  # Allow very deep nesting (e.g., mm/3de/mm-default/scenes/scene/FG01/)
                                        "-type",
                                        "f",
                                        "(",
                                        "-name",
            ...
            if has_publish_dir:
                ...
                try:
                    ...
                    find_cmd = [
                        ...
                        str(publish_dir),
                        "-maxdepth",
                        "15",  # Allow deep nesting in publish
                        "-type",
                        "f",
                        "(",
                        "-name",
    ...
    def find_all_3de_files_in_show(
        ...
        try:
            ...
            parallel_sequences = (
                Config.THREEDE_SCAN_PARALLEL_SEQUENCES
                if hasattr(Config, "THREEDE_SCAN_PARALLEL_SEQUENCES")
                else 1
            )
            ...
            if len(search_paths) > 1 and parallel_sequences > 1:
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    future_to_path = {
                        executor.submit(find_in_path, path): path
                        for path in search_paths
                    }
