...
from launcher_manager import LauncherManager, LauncherEnvironment, LauncherTerminal
...
def main():
    ...
    complex_id = manager.create_launcher(
        ...
        variables={
            "script_template": "/studio/templates/nuke/shot_template.nk"
        },
        environment=LauncherEnvironment(
            type="rez",
            packages=["nuke", "studio_tools"]
        ),
        terminal=LauncherTerminal(
            required=False,
            title="Nuke - {show} {full_name}"
        )
    )
    ...
    if simple_id:
        ...
        success = manager.execute_in_shot_context(
            simple_id, test_shot, dry_run=True
        )
    ...
    if debug_id:
        ...
        success = manager.update_launcher(
            debug_id,
            description="Enhanced debug tools for shot analysis with logging"
        )
