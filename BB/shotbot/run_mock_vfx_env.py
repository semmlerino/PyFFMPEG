        ...
        try:
            ...
            if structure_file.exists():
                ...
                if result.returncode == 0:
                    ...
                    logger.error(f"❌ Failed to create mock environment: {result.stderr}")
                    logger.info("Manual command: python recreate_vfx_structure.py vfx_structure_complete.json")
                    return False
            else:
        ...
        if demo_shots_file.exists():
            ...
            for shot in demo_data.get("shots", []):
                if "path" in shot and shot["path"].startswith("/shows/"):
                    shot["path"] = shot["path"].replace("/shows/", "/tmp/mock_vfx/shows/")
                    updated = True
...
def main():
    ...
    if not create_shows_symlink():
        logger.warning("⚠️  Could not create /shows symlink, paths may not work correctly")
