...
def test_regex_pattern_caching():
    ...
    if optimized_time > 0:
        ...
        if percent_improvement >= min_improvement:
            print(f"✓ Improvement test PASSED: {percent_improvement:.1f}% >= {min_improvement}%")
            improvement_passed = True
        else:
            print(f"✗ Improvement test FAILED: {percent_improvement:.1f}% < {min_improvement}%")
        ...
        if speedup_passed and improvement_passed:
            ...
            print(f"  Expected at least {min_speedup}x speedup and {min_improvement}% improvement")
            return False
    else:
        ...
        try:
            ...
            if cache_size_after_first == 1 and cache_size_after_second == 1:
                ...
                if patterns1 is patterns2:
                    ...
                    print("! Patterns are different objects (but may still be functionally correct)")
                
                return True
            else:
                print(f"! Unexpected cache sizes: {cache_size_after_first}, {cache_size_after_second}")
