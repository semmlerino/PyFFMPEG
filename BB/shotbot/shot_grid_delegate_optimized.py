...
class ShotGridDelegateOptimized(QStyledItemDelegate):
    ...
    def paint(
        ...
        buffer_painter.setRenderHint(QPainter.RenderHint.Antialiasing, self._quality_mode != "low")
    ...
    def _paint_to_buffer(
        ...
        self._paint_background_optimized(painter, option.rect, is_selected, is_hover, metrics)
        ...
        if isinstance(thumbnail, QPixmap) and not thumbnail.isNull():
            ...
            self._paint_loading_optimized(painter, metrics.thumbnail_rect, shot_name, loading_progress)
        else:
    ...
    def _paint_focus_optimized(
        ...
        painter.drawRoundedRect(focus_rect, self._border_radius - 2, self._border_radius - 2)
    ...
    def _calculate_metrics(self) -> PaintMetrics:
        ...
        # Border path
        border_path = QPainterPath()
        border_path.addRoundedRect(QRectF(item_rect), self._border_radius, self._border_radius)
    ...
    def _update_animations(self) -> None:
        ...
        for item_id, animation in self._loading_animations.items():
            if animation.last_update.elapsed() > 5000:  # Remove after 5 seconds inactive
