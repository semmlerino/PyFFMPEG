...
class PerformanceProfiler:
    ...
    def profile_ui_rendering(self):
        ...
        for i in range(432):
            shot = Shot(
                ...
                sequence="test_seq",
                shot=f"shot_{i:04d}",
                workspace_path=f"/mock/shows/test_show/test_seq/shot_{i:04d}",
            )
        ...
        with self.profile_cpu("shot_grid_population"):
            with self.profile_memory("shot_grid_population"):
                ...
                print(f"Shot grid populated with {len(mock_shots)} shots")
        ...
        with self.profile_cpu("qpixmap_operations"):
            with self.profile_memory("qpixmap_operations"):
                ...
                print(f"Created and painted {len(pixmaps)} QPixmaps")
                # Cleanup
                pixmaps.clear()
    ...
    def profile_memory_patterns(self):
        ...
        with self.profile_cpu("memory_limit_enforcement"):
            with self.profile_memory("memory_limit_enforcement"):
                ...
                print(f"Memory manager evicted {evicted_total} items total")
                print(
                    f"Final memory usage: {memory_manager.memory_usage_bytes / 1024 / 1024:.2f}MB"
                )
    ...
    def profile_io_operations(self):
        ...
        with self.profile_cpu("filesystem_scanning"):
            with self.profile_memory("filesystem_scanning"):
                ...
                previous_shots_count = 0
                try:
                    previous_shots = list(finder.find_previous_shots())
                    previous_shots_count = len(previous_shots)
                except Exception as e:
                    print(f"Previous shots scanning error: {e}")

                print(f"Found {previous_shots_count} previous shots")
        ...
        with self.profile_cpu("subprocess_communication"):
            try:
                ...
                print(f"Subprocess result length: {result_len}")
            except Exception as e:
    ...
    def profile_cache_performance(self):
        ...
        with self.profile_cpu("shot_cache_operations"):
            ...
            print(f"Shot cache: {cached_shots} items cached, hit rate: {hit_rate:.1f}%")
        ...
        with self.profile_cpu("threede_cache_operations"):
            ...
            for i in range(50):
                ...
                try:
                    ...
                    print(f"3DE cache error: {e}")

            print(f"3DE cache: {cached_scenes} scenes cached")
    ...
    def profile_thread_pool_optimization(self):
        ...
        with self.profile_cpu("thread_pool_executor"):
            with self.profile_memory("thread_pool_executor"):
                ...
                print(
                    f"ThreadPoolExecutor: Completed {len(results)}/100 tasks with {max_workers} workers"
                )
    ...
    def print_results(self):
        ...
        for operation, data in self.results.items():
            ...
            if "current_mb" in data:
                memory = data["current_mb"]
                peak = data["peak_mb"]
                total_memory_mb += peak
                print(f"  Memory Usage: {memory:.2f}MB")
                print(f"  Peak Memory: {peak:.2f}MB")

        print("\nSUMMARY:")
        print(f"  Total CPU Time: {total_duration:.3f}s")
        print(f"  Total Peak Memory: {total_memory_mb:.2f}MB")
