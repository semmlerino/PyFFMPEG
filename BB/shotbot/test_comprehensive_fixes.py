...
def test_temp_folder_consistency():
    ...
    old_folders = [
        ...
        if f.startswith("shotbot_bundle_") and f != "shotbot_bundle_temp"
    ]
    ...
    if os.path.exists(expected_path):
        ...
        logger.info("✓ Cleaned up existing temp directory")
...
def test_session_creation_with_debugging():
    ...
    try:
        ...
        from debug_utils import SystemDiagnostics, state_tracker, timing_profiler
        ...
        logger.info(
            f"  Sessions created: {len(metrics.get('sessions', {}).get('workspace', {}).get('sessions', []))}"
        )
        ...
        for session_id in ["workspace_0", "workspace_1", "workspace_2"]:
            ...
            if history:
                ...
                for _, from_state, to_state, reason in history[
                    -3:
                ]:  # Last 3 transitions
                    logger.info(
                        f"    {from_state} → {to_state} {f'[{reason}]' if reason else ''}"
                    )
