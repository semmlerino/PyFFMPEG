...
class AccessibilityManager:
    ...
    @staticmethod
    def setup_shot_grid_accessibility(grid_widget: QWidget, grid_type: str = "shots") -> None:
        ...
        grid_widget.setAccessibleDescription(descriptions.get(grid_type, "Grid of shots"))
    ...
    @staticmethod
    def setup_launcher_buttons_accessibility(app_buttons: Dict[str, QPushButton]) -> None:
    ...
    def setup_comprehensive_tooltips(window: Any) -> None:
        ...
        if hasattr(window, 'refresh_action'):
            window.refresh_action.setToolTip("Refresh shot list from workspace (Ctrl+R)")
        ...
        if hasattr(window, 'reset_layout_action'):
            window.reset_layout_action.setToolTip("Reset window layout to default configuration")
    ...
    def setup_keyboard_navigation(window: Any) -> None:
        ...
        if all(hasattr(window, attr) for attr in ['tab_widget', 'shot_grid', 'app_buttons']):
            ...
            if hasattr(window.shot_grid, 'list_view'):
                window.setTabOrder(window.shot_grid.size_slider, window.shot_grid.list_view)
            
            # Then launcher buttons in order
            prev_widget = window.shot_grid.list_view if hasattr(window.shot_grid, 'list_view') else window.shot_grid
    ...
    @staticmethod
    def setup_validation_feedback_accessibility(widget: QWidget, is_valid: bool, message: str) -> None:
