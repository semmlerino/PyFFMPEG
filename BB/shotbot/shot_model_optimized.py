...
class OptimizedShotModel(BaseShotModel):
    ...
    def __init__(self, cache_manager: Optional['CacheManager'] = None, load_cache: bool = True) -> None:
        super().__init__(cache_manager, load_cache)
        
        # Background loader with thread safety
        self._async_loader: Optional[AsyncShotLoader] = None
    ...
    def _on_shots_loaded(self, new_shots: List[Shot]) -> None:
        ...
        if has_changes:
            ...
            logger.info(f"Background load complete: {old_count} → {len(new_shots)} shots")
            ...
            logger.info(f"Background load complete: no changes ({len(new_shots)} shots)")
    ...
    def refresh_strategy(self) -> RefreshResult:
        ...
        if not self.shots and not loading:
            ...
            # For subsequent refreshes, start background refresh
            self._start_background_refresh()
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics.update({
            ...
            "cache_hit_rate": self._cache_hit_count / max(1, self._cache_hit_count + self._cache_miss_count),
        ...
        if self._async_loader:
            if self._async_loader.isRunning():
                ...
                if not self._async_loader.wait(2000):
                    logger.warning("Background loader did not stop gracefully within 2s")
                    ...
                    if not self._async_loader.wait(2000):
                        ...
                        logger.error("Background loader thread abandoned - will be cleaned on exit")
...
def create_optimized_shot_model(cache_manager: Optional['CacheManager'] = None) -> OptimizedShotModel:
    ...
    model.shots_loaded.connect(lambda shots: logger.info(f"UI can display {len(shots)} shots"))
    model.shots_changed.connect(lambda shots: logger.info(f"UI should update to {len(shots)} shots"))
