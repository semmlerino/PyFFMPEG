...
class PersistentTerminalManager(QObject):
    ...
    def __init__(self, fifo_path: str | None = None, dispatcher_path: str | None = None) -> None:
        ...
        logger.info(f"PersistentTerminalManager initialized with FIFO: {self.fifo_path}")
    ...
    def _launch_terminal(self) -> bool:
        ...
        terminal_commands = [
            ...
            ["gnome-terminal", "--title=ShotBot Terminal", "--", "bash", self.dispatcher_path, self.fifo_path],
            # konsole
            ["konsole", "--title", "ShotBot Terminal", "-e", "bash", self.dispatcher_path, self.fifo_path],
            # xterm
            ["xterm", "-title", "ShotBot Terminal", "-e", "bash", self.dispatcher_path, self.fifo_path],
        ...
        for cmd in terminal_commands:
            try:
                ...
                if self._is_terminal_alive():
                    logger.info(f"Terminal launched successfully with PID: {self.terminal_pid}")
