...
def test_imageio_exr_reading():
    ...
    for i, exr_path in enumerate(test_exr_files):
        ...
        try:
            ...
            print(
                f"✅ Success! Image shape: {img_array.shape}, dtype: {img_array.dtype}"
            )
            ...
            try:
                ...
                print(
                    f"✅ v2 API Success! Image shape: {img_array.shape}, dtype: {img_array.dtype}"
                )
                success_count += 1
...
def test_system_tool_conversion():
    ...
    for tool_cmd in tools_to_test:
        ...
        try:
            result = subprocess.run(
                tool_cmd, capture_output=True, text=True, timeout=30
            )
    ...
    if script_path.exists() and test_file.endswith(".exr"):
        ...
        try:
            result = subprocess.run(
                [str(script_path), test_file, "/tmp/script_test.jpg", "256"],
                capture_output=True,
                text=True,
                timeout=60,
            )
            ...
            if result.returncode == 0:
                print("✅ Conversion script successful!")
