                ...
                if node.module == 'typing':
                    for alias in node.names:
                        if alias.name == 'override':
                            issues.append(f"Line {node.lineno}: imports 'override' from 'typing' (use typing_extensions)")
...
def main():
    ...
    for pattern in ['*.py', '**/*.py']:
        for path in Path('.').glob(pattern):
            if ('venv' not in str(path) and 'test_venv' not in str(path) and 
                '.pyenv' not in str(path) and 'build' not in str(path)):
