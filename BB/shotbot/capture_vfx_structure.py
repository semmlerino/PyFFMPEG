...
def scan_directory(path: Path, base_path: Path, max_depth: int = 10, 
    ...
    if path.is_file():
        ...
        important_dirs = {
            'publish', 'editorial', 'cutref', 'turnover', 'plate',
            'user', '3de', 'scenes', 'scene', 'exports', 'jpg',
            '1920x1080', 'mm', 'mm-default', 'shots'
        }
        
        # File patterns we care about
        important_patterns = {
            '.3de', '.jpg', '.jpeg', '.png', '.exr', '.nk',
            'thumbnail', 'poster_frame', 'frame'
        }
        ...
        try:
            ...
            with os.scandir(path) as entries:
                for entry in entries:
                    ...
                    is_important = (
                        entry.name.lower() in important_dirs or
                        any(pattern in entry.name.lower() for pattern in important_patterns) or
                        current_depth < 3  # Always include top levels
                    )
...
def capture_structure(shows: List[str] = None):
    ...
    print(f"Found {len(workspace_shots)} workspace shots from shows: {', '.join(workspace_shows)}", 
          file=sys.stderr)
    ...
    if shows:
        ...
        target_shows = workspace_shows if workspace_shows else ['gator', 'jack_ryan', 'broken_eggs']
    ...
    structure['patterns'] = {
        ...
        '3de_paths': [
            "user/*/mm/3de/mm-default/scenes/scene/*/*.3de"
        ],
        'plate_paths': [
            "publish/turnover/plate/input_plate/*/*/*.exr"
        ]
    }
...
def main() -> None:
    ...
    if args.stdout:
        ...
        print(output)
        print(f"\nCapture complete! Found {len(structure['shows'])} shows", file=sys.stderr)
    else:
