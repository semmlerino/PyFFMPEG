...
class ThumbnailBottleneckProfiler:
    ...
    def profile_thumbnail_discovery_pipeline(self) -> Dict[str, Any]:
        ...
        for show, sequence, shot in test_shots:
            ...
            thumbnail_dir = PathUtils.build_thumbnail_path(Config.SHOWS_ROOT, show, sequence, shot)
            editorial_exists = PathUtils.validate_path_exists(thumbnail_dir, "Thumbnail dir")
        ...
        return {
            ...
            "avg_time_per_shot_ms": (total_time / len(test_shots) * 1000) if test_shots else 0,
            "filesystem_operations": filesystem_operations,
            "fs_ops_per_shot": filesystem_operations / len(test_shots) if test_shots else 0,
            "stage_performance": stage_performance,
            "cache_growth": cache_growth,
            "cache_effectiveness": cache_effectiveness,
            "thumbnails_per_second": len(test_shots) / total_time if total_time > 0 else 0,
            "bottleneck_stage": max(stage_performance.keys(), 
                                  key=lambda k: stage_performance[k]["avg_time_ms"]) if stage_performance else "unknown"
        }
    ...
    def profile_memory_usage_patterns(self, duration: int = 30) -> Dict[str, Any]:
        ...
        while time.time() - start_time < duration:
            ...
            if sample_count % 10 == 0:  # Every 10th sample
                ...
                for j in range(5):
                    ...
                    cached_thumbnails[key] = f"fake_thumbnail_data_{j}" * 100  # ~2KB each
        ...
        if shot_objects:
            shot_memory_estimate = (memory_growth / len(shot_objects)) if memory_growth > 0 else 0
        else:
        ...
        return {
            ...
            "memory_efficiency_rating": "Good" if memory_growth < 10 else "Fair" if memory_growth < 50 else "Poor",
            "peak_memory_overhead_percent": ((peak_memory - initial_memory) / initial_memory * 100) if initial_memory > 0 else 0
        }
    ...
    def profile_ui_responsiveness(self) -> Dict[str, Any]:
        ...
        test_operations = [
            ...
            ("Thumbnail discovery batch", lambda: self._test_thumbnail_discovery_batch()),
        ...
        for op_name, op_func in test_operations:
            ...
            blocking_operations.append({
                ...
                "severity": "High" if elapsed > 0.1 else "Medium" if elapsed > 0.05 else "Low"
            })
        
        # Calculate UI impact
        total_blocking_time = sum(op["duration_ms"] for op in blocking_operations if op["blocks_ui"])
        high_severity_ops = [op for op in blocking_operations if op["severity"] == "High"]
        ...
        return {
            ...
            "ui_responsiveness_rating": "Good" if total_blocking_time < 50 else "Fair" if total_blocking_time < 200 else "Poor",
    ...
    def _test_shot_parsing_batch(self):
        ...
        paths = [f"/shows/Show{i}/shots/0{i:02d}/0{i:02d}_00{j:02d}/user/test" 
    ...
    def _generate_ui_recommendations(self, blocking_operations: List[Dict[str, Any]]) -> List[str]:
        ...
        for op in blocking_operations:
            if op["blocks_ui"]:
                if "batch" in op["operation"].lower():
                    recommendations.append(f"Move {op['operation']} to background thread")
                if op["duration_ms"] > 100:
                    recommendations.append(f"Optimize {op['operation']} - taking {op['duration_ms']:.1f}ms")
        
        high_severity = [op for op in blocking_operations if op["severity"] == "High"]
        if high_severity:
            recommendations.append(f"Priority: {len(high_severity)} high-severity blocking operations detected")
    ...
    def run_comprehensive_analysis(self, memory_duration: int = 15) -> Dict[str, Any]:
        ...
        try:
            ...
            results["shot_name_extraction"] = self.profile_shot_name_extraction_overhead()
            ...
            results["memory_patterns"] = self.profile_memory_usage_patterns(memory_duration)
    ...
    def generate_bottleneck_report(self, results: Dict[str, Any]) -> str:
        ...
        if "shot_name_extraction" in results:
            ops_per_sec = results["shot_name_extraction"].get("operations_per_second_current", 0)
            if ops_per_sec < 100000:  # Less than 100k ops/sec
                issues.append(f"Shot name extraction only {ops_per_sec:.0f} ops/sec - significant bottleneck")
        ...
        if "thumbnail_discovery" in results:
            ...
            if avg_time_ms > 20:  # More than 20ms per shot
                issues.append(f"Thumbnail discovery {avg_time_ms:.1f}ms per shot - will impact startup")
        
        if "memory_patterns" in results:
            growth = results["memory_patterns"].get("memory_growth_mb", 0)
            if growth > 20:
                issues.append(f"Memory growing {growth:.1f}MB during test - potential leak")
        
        if "ui_responsiveness" in results:
            blocking_time = results["ui_responsiveness"].get("total_blocking_time_ms", 0)
            if blocking_time > 100:
                issues.append(f"UI blocking operations total {blocking_time:.1f}ms - will freeze interface")
        ...
        if "shot_name_extraction" in results:
            ...
            report.append("-" * 32)
            report.append(f"  Current performance: {data.get('operations_per_second_current', 0):.0f} ops/sec")
            report.append(f"  Time per operation: {data.get('current_per_operation_us', 0):.2f} microseconds")
            report.append(f"  Potential improvement: {data.get('improvement_percent', 0):.1f}%")
            report.append(f"  Startup impact: {data.get('startup_impact_estimate', 'N/A')}")
            report.append("")
        ...
        if "thumbnail_discovery" in results:
            ...
            report.append("-" * 39)
            report.append(f"  Average time per shot: {data.get('avg_time_per_shot_ms', 0):.1f}ms")
            report.append(f"  Filesystem operations per shot: {data.get('fs_ops_per_shot', 0):.1f}")
            report.append(f"  Bottleneck stage: {data.get('bottleneck_stage', 'unknown')}")
            
            if "stage_performance" in data:
                report.append("  Stage breakdown:")
                for stage, perf in data["stage_performance"].items():
                    report.append(f"    {stage}: {perf.get('avg_time_ms', 0):.2f}ms avg, {perf.get('calls', 0)} calls")
            report.append("")
        ...
        if "memory_patterns" in results:
            ...
            report.append(f"  Memory per shot object: ~{data.get('estimated_memory_per_shot_kb', 0):.1f}KB")
            report.append(f"  Memory trend: {data.get('memory_trend', 'unknown')}")
            report.append(f"  Efficiency rating: {data.get('memory_efficiency_rating', 'unknown')}")
            report.append("")
        ...
        if "ui_responsiveness" in results:
            ...
            report.append("-" * 27)
            report.append(f"  Total blocking time: {data.get('total_blocking_time_ms', 0):.1f}ms")
            report.append(f"  High severity operations: {data.get('high_severity_operations', 0)}")
            report.append(f"  Responsiveness rating: {data.get('ui_responsiveness_rating', 'unknown')}")
            
            if "blocking_operations" in data:
                report.append("  Blocking operations:")
                for op in data["blocking_operations"]:
                    if op["blocks_ui"]:
                        report.append(f"    • {op['operation']}: {op['duration_ms']:.1f}ms ({op['severity']} severity)")
        ...
        if "shot_name_extraction" in results and results["shot_name_extraction"].get("improvement_percent", 0) > 20:
            priority_fixes.append("HIGH: Implement optimized regex pattern for shot name extraction")
        
        if "thumbnail_discovery" in results:
            fs_ops = results["thumbnail_discovery"].get("fs_ops_per_shot", 0)
            if fs_ops > 3:
                priority_fixes.append("HIGH: Reduce filesystem operations through better caching")
            
            bottleneck = results["thumbnail_discovery"].get("bottleneck_stage", "")
            if bottleneck:
                priority_fixes.append(f"MEDIUM: Optimize {bottleneck} stage of thumbnail discovery")
        
        if "ui_responsiveness" in results and results["ui_responsiveness"].get("total_blocking_time_ms", 0) > 50:
            priority_fixes.append("HIGH: Move blocking operations to background threads")
        
        if "memory_patterns" in results and results["memory_patterns"].get("memory_growth_mb", 0) > 10:
            priority_fixes.append("MEDIUM: Investigate memory growth and implement cleanup")
        
        for i, fix in enumerate(priority_fixes, 1):
            report.append(f"{i}. {fix}")
        
        if not priority_fixes:
            report.append("✅ No high-priority optimizations needed - performance is acceptable")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Profile ShotBot thumbnail discovery bottlenecks")
    parser.add_argument("--memory-duration", type=int, default=15, 
