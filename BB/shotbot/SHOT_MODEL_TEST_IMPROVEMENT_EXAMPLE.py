...
class TestShotModelImproved:
    ...
    def test_refresh_shots_real_parsing_logic(self, shot_model_real, mock_workspace_structure):
        ...
        mocked away.
        """
        with patch.object(shot_model_real._process_pool, 'execute_workspace_command') as mock_execute:
            ...
            seq010_0010 = next((s for s in shots if s.shot == "0010" and s.sequence == "seq010"), None)
    ...
    def test_shot_thumbnail_discovery_real_filesystem(self, shot_model_real, tmp_path):
        ...
        editorial_dir = show_path / "publish" / "editorial" / "cutref" / "v001" / "jpg" / "1920x1080"
        editorial_dir.mkdir(parents=True)
        editorial_file = editorial_dir / "frame.1001.jpg"
        editorial_file.write_bytes(b'\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00\xff\xd9')
    ...
    def test_cache_integration_real_persistence(self, shot_model_real, tmp_path):
        ...
        new_cache_manager = CacheManager(cache_dir=shot_model_real.cache_manager.cache_dir)
...
if __name__ == "__main__":
    ...
    """
    print("Run with: python -m pytest SHOT_MODEL_TEST_IMPROVEMENT_EXAMPLE.py --cov=shot_model --cov-report=term-missing")