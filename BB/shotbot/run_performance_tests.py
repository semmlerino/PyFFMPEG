...
def extract_performance_metrics(output: str) -> Dict[str, str]:
    ...
    for line in lines:
        ...
        if 'cache' in line.lower() and ('entries' in line.lower() or 'size' in line.lower()):
            try:
                import re
...
def generate_performance_report(results: List[Tuple[str, bool, float, str]]) -> str:
        ...
        if 'speedup' in metrics:
            try:
                ...
                if speedup_value > 1.5:  # Significant improvement
                    report.append(f"✓ {test_name}: {metrics['speedup']} performance improvement")
        ...
        if 'improvement' in metrics:
            try:
                ...
                if improvement_value > 50:  # Significant improvement
                    report.append(f"✓ {test_name}: {metrics['improvement']} performance improvement")
                    optimization_found = True
            except Exception:
                pass
    ...
    if not optimization_found:
        ...
        report.append("  This may indicate that optimizations are not working as expected")
    ...
    if passed_tests == total_tests and optimization_found:
        ...
        report.append("! Consider running tests in isolation to identify specific issues")
...
def main():
    ...
    for script_name in test_scripts:
        script_path = project_dir / script_name
        test_name = script_name.replace("standalone_", "").replace("_performance_test.py", "").replace("_", " ").title()
