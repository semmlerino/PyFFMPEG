...
def test_individual_file(test_file, timeout=10):
    ...
    try:
        ...
        has_fixture_error = "fixture" in output.lower() and "not found" in output.lower()
        
        return {
            "file": test_file,
            "status": "timeout" if has_timeout else ("success" if result.returncode == 0 else "failure"),
            "duration": duration,
            "passed": passed,
            "failed": failed,
            "skipped": skipped,
...
def main():
    ...
    total = len(results)
    successful = sum(1 for r in results if r["status"] == "success" and r["skipped"] == 0)
    ...
    for result in results:
        status_icon = "✓" if result["status"] == "success" and result["skipped"] == 0 else "✗"
