        ...
        with mock.patch.dict(os.environ, {"SHOWS_ROOT": shows_root}):
            ...
            try:
                ...
                if shows_root_escaped in pattern:
                    print(f"  ✓ shot_finder_base.py: Pattern contains {shows_root_escaped}")
                    success_count += 1
                else:
                    print(f"  ✗ shot_finder_base.py: Pattern missing {shows_root_escaped}")
                    print(f"    Got: {pattern}")
            ...
            try:
                ...
                # Check pattern contains escaped SHOWS_ROOT
                if shows_root_escaped in pattern:
                    print(f"  ✓ base_shot_model.py: Pattern contains {shows_root_escaped}")
                    success_count += 1
                else:
                    print(f"  ✗ base_shot_model.py: Pattern missing {shows_root_escaped}")
                    print(f"    Got: {pattern}")
        ...
        with mock.patch('pathlib.Path', return_value=Path(temp_path)):
            with mock.patch('pathlib.Path.exists', return_value=True):
                ...
                with mock.patch.object(logger, 'error') as mock_error:
                    pool = create_mock_pool_from_filesystem()
                    if any('JSONDecodeError' in str(call) for call in mock_error.call_args_list):
    ...
    try:
        with mock.patch('pathlib.Path.__new__', return_value=Path(temp_path)):
            with mock.patch('pathlib.Path.exists', return_value=True):
                with mock.patch.object(logger, 'error') as mock_error:
                    pool = create_mock_pool_from_filesystem()
                    if any('Expected dict' in str(call) for call in mock_error.call_args_list):
    ...
    try:
        with mock.patch('pathlib.Path.__new__', return_value=Path(temp_path)):
            with mock.patch('pathlib.Path.exists', return_value=True):
                with mock.patch.object(logger, 'error') as mock_error:
                    pool = create_mock_pool_from_filesystem()
                    if any("Missing 'shots' key" in str(call) for call in mock_error.call_args_list):
    ...
    try:
        with mock.patch('pathlib.Path.__new__', return_value=Path(temp_path)):
            with mock.patch('pathlib.Path.exists', return_value=True):
                with mock.patch.object(logger, 'error') as mock_error:
                    pool = create_mock_pool_from_filesystem()
                    if any('missing fields' in str(call) for call in mock_error.call_args_list):
