...
def test_import_method(module_name, description, extra_paths=None):
    ...
    try:
        ...
        print(f"   Module attributes: {[attr for attr in dir(module) if not attr.startswith('_')][:10]}...")
        
        # Test specific attributes we need
        if hasattr(module, 'InputFile'):
            print(f"   ✅ Has InputFile")
        if hasattr(module, 'PixelType'):
            print(f"   ✅ Has PixelType")
    ...
    try:
        for item in path.iterdir():
            if item.is_dir():
                ...
                if (item / '__init__.py').exists() or any(f.suffix == '.so' for f in item.glob('*')):
                    packages.append(str(item))
                packages.extend(_find_packages_recursive(item, max_depth, current_depth + 1))
    except PermissionError:
        pass
    except Exception as e:
...
def main():
    ...
    rez_vars = {k: v for k, v in os.environ.items() if 'REZ' in k or 'openexr' in k.lower()}
    ...
    if not openexr_module and python_packages:
        ...
        for pkg_path in python_packages[:5]:  # Test first 5 paths
            ...
            openexr_module = test_import_method('OpenEXR', f'OpenEXR with path {pkg_parent}', [pkg_parent])
            if openexr_module:
                break
            openexr_module = test_import_method('openexr', f'openexr with path {pkg_parent}', [pkg_parent])
            if openexr_module:
    ...
    if not imath_module and python_packages:
        for pkg_path in python_packages[:5]:
            pkg_parent = str(Path(pkg_path).parent)
            imath_module = test_import_method('Imath', f'Imath with path {pkg_parent}', [pkg_parent])
            if imath_module:
                break
    ...
    try:
        ...
        print(f"Available plugins: {list(plugins.keys()) if hasattr(plugins, 'keys') else 'Unknown'}")
