...
class ThreadSafeProgressTracker:
    ...
    def __init__(self, progress_callback: Optional[Callable[[int, str], None]] = None, 
        ...
        logger.debug(f"ThreadSafeProgressTracker {self._id} created with interval={update_interval}")
    
    def update_worker_progress(self, worker_id: str, new_count: int, status: str = "") -> None:
        ...
        if should_report and self._progress_callback:
            try:
                aggregated_status = status or f"Processing... ({len(self._worker_progress)} workers active)"
                self._progress_callback(total_progress, aggregated_status)
            except Exception as e:
                logger.error(f"ThreadSafeProgressTracker {self._id} callback error: {e}")
    ...
    def mark_worker_completed(self, worker_id: str) -> None:
        ...
        logger.debug(f"ThreadSafeProgressTracker {self._id} worker {worker_id} completed")
    ...
    def force_progress_report(self, status: str = "") -> None:
        ...
        try:
            ...
            logger.error(f"ThreadSafeProgressTracker {self._id} force report error: {e}")
    ...
    def _shutdown_executor(self) -> None:
        ...
        try:
            ...
            while time.time() - start_time < self.shutdown_timeout:
                ...
                # Timeout reached
                logger.warning(f"Executor shutdown timeout after {self.shutdown_timeout}s, "
...
def integrate_with_existing_parallel_scan():
    ...
    def fixed_parallel_scan_pattern(work_chunks, scan_function, cancel_flag=None, max_workers=4):
        """Fixed version of parallel scanning with proper cancellation."""
        
        # Step 1: Create cancellation context
        cancel_event, pool_manager = create_cancellation_context(max_workers=max_workers)
