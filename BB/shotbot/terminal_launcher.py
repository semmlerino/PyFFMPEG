...
class TerminalLauncher(QObject):
    ...
    def execute_launcher(
        ...
        try:
            ...
            final_command = self._substitute_variables(launcher.command, variables or {})
            ...
            if result.success:
                ...
                logger.info(f"Successfully executed launcher '{launcher.name}': {final_command}")
            else:
                timestamp = self._get_timestamp()
                self.launcher_failed.emit(launcher.name, result.error_message, timestamp)
                logger.error(f"Failed to execute launcher '{launcher.name}': {result.error_message}")
    ...
    def _build_macos_terminal_command(
        ...
        if working_directory:
            script_parts.append(f'tell application "Terminal" to do script "cd \\"{working_directory}\\" && {command}"')
        else:
