...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'debug_crash_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
logger.info(f"Environment: SHOTBOT_DEBUG_VERBOSE={os.environ.get('SHOTBOT_DEBUG_VERBOSE')}")
logger.info("=" * 80)
...
try:
    ...
    logger.info(f"  Average response time: {metrics.get('average_response_ms', 0):.2f}ms")
    ...
    if 'sessions' in metrics:
        for pool_type, pool_info in metrics['sessions'].items():
            ...
            for session in pool_info.get('sessions', []):
                logger.info(f"    - {session.get('session_id')}: alive={session.get('alive')}, "
