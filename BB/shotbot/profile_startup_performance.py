...
def main():
    ...
    print(f"   Async UI display: {results['strategies'].get('async_ui_display', -1):.3f}s")
    print(f"   Cached parse time: {results['strategies'].get('cached_parse_time', -1):.6f}s")
    ...
    if "shot_model" in results and "top_functions" in results["shot_model"]:
        ...
        for func in results["shot_model"]["top_functions"][:5]:
            print(f"  {func['cumulative_time']:.3f}s - {func['function']} ({func['calls']} calls)")
