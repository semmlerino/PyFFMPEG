    ...
    def run(self):
        ...
        try:
            ...
            if not pixmap.isNull():
                ...
                self.signals.loading_failed.emit(
                    self.task.row, "Invalid image format"
                )
...
class ShotItemModelOptimized(QAbstractListModel):
    ...
    def canFetchMore(self, parent: QModelIndex = QModelIndex()) -> bool:
        ...
        if parent.isValid():
            return False
        return (
            self._fetch_more_enabled and 
            len(self._loaded_shots) < len(self._all_shots)
        )
        ...
        if role == ShotRole.IsSelectedRole:
            if value:
                ...
                if old_selection.isValid():
                    ...
                    self.dataChanged.emit(old_index, old_index, [ShotRole.IsSelectedRole])
    ...
    def set_visible_range(self, start: int, end: int) -> None:
        ...
        self._cancel_tasks_outside_range(
            start - self.PREFETCH_BUFFER,
            end + self.PREFETCH_BUFFER
        )
    ...
    def enable_virtual_proxy(self, enabled: bool) -> None:
        ...
        if not enabled and len(self._loaded_shots) < len(self._all_shots):
            ...
            self.beginInsertRows(
                QModelIndex(),
                len(self._loaded_shots),
                len(self._all_shots) - 1
            )
