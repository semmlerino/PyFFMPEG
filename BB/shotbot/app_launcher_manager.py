...
class AppLauncherManager(LoggingMixin, QObject):
    ...
    def launch_app_with_shot(self, app_name: str, shot: Shot | None = None) -> bool:
        ...
        try:
            ...
            self.status_update.emit(
                f"Launching {app_name} for shot {shot.full_name}..."
            )
    ...
    def launch_app_with_scene(self, app_name: str, scene: ThreeDEScene) -> bool:
        ...
        try:
            ...
            self.launch_started.emit(app_name, context)
            self.status_update.emit(
                f"Launching {app_name} with 3DE scene {scene.scene_path.name}..."
            )
    ...
    def _execute_app_launch(
        self, app_name: str, shot: Shot | None, scene: ThreeDEScene | None
    ) -> bool:
