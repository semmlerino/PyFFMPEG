...
class LauncherConfig:
    ...
    def _ensure_directories(self):
        ...
        try:
            ...
            raise LauncherConfigError(f"Failed to create config directory {self.config_dir}: {e}")
    ...
    def _load_config_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise LauncherConfigError("Configuration file must contain a JSON object")
            ...
            logger.debug(f"Loaded configuration with {len(data.get('launchers', []))} launchers")
            return data
    ...
    def _migrate_config(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Migrating configuration from version {current_version} to {self.SCHEMA_VERSION}")
        ...
        try:
            ...
            for launcher in launchers:
                try:
                    ...
                    if errors:
                        logger.error(f"Launcher '{launcher.id}' validation errors: {errors}")
                        continue
    ...
    def add_launcher(self, launcher: Launcher):
        ...
        if any(l.id == launcher.id for l in launchers):
            raise LauncherConfigError(f"Launcher with ID '{launcher.id}' already exists")
