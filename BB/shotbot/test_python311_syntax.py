...
def test_python311_compatibility():
    ...
    for pattern in ['*.py', '**/*.py']:
        for path in Path('.').glob(pattern):
            if 'venv' not in str(path) and 'test_venv' not in str(path) and '.pyenv' not in str(path):
                    ...
                    if node.module == 'typing':
                        for alias in node.names:
                            if alias.name == 'override':
                                errors.append(f"{filepath}: imports 'override' from typing (use typing_extensions)")
    ...
    if errors:
        ...
        print("  • f-string format specs: {value:.1f} syntax is Python 3.6+ (compatible) ✓")
