...
class TestPlatePriorityValidation:
    ...
    def test_undistortion_plate_priority_consistency(self) -> None:
        ...
        for i, plate in enumerate(common_plates[:-1]):
            ...
            assert plate in undistortion, (
                f"Plate {plate} missing from UNDISTORTION_PLATE_PRIORITY"
            )
            assert next_plate in undistortion, (
                f"Plate {next_plate} missing from UNDISTORTION_PLATE_PRIORITY"
            )
    ...
    def test_plate_priority_order_legacy_compatibility(self) -> None:
        ...
        for plate, priority in legacy.items():
            ...
            assert priority > 0, (
                f"Legacy plate {plate} has non-positive priority {priority}"
            )
...
class TestTimeoutConfigurationValidation:
    ...
    def test_threading_config_timeouts_are_positive(self) -> None:
        ...
        for name, timeout in timeouts:
            ...
            assert timeout > 0, (
                f"ThreadingConfig.{name} must be positive, got {timeout}"
            )
