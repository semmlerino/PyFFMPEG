...
class TestScriptVersionDetection:
    ...
    def test_find_existing_scripts_returns_versions(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        result = PlateDiscovery.find_existing_scripts(
            str(workspace_path), shot_name, plate_name
        )
    ...
    def test_find_existing_scripts_sorted_by_version(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        for version in [10, 2, 5, 1]:
            (
                script_dir
                / f"{shot_name}_mm-default_{plate_name}_scene_v{version:03d}.nk"
            ).touch()
    ...
    def test_get_next_script_version_increments(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        result = PlateDiscovery.get_next_script_version(
            str(workspace_path), shot_name, plate_name
        )
    ...
    def test_get_next_script_version_with_no_scripts(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestScriptPathConstruction:
    ...
    def test_get_script_directory(self, tmp_path: Path) -> None:
        ...
        result = PlateDiscovery.get_plate_script_directory(
            str(workspace_path), plate_name
        )
    ...
    def test_construct_script_path(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_script_versioning_format(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        for path, version in result:
            ...
            assert path.name.endswith(expected_suffix), (
                f"Script should end with {expected_suffix}"
            )
...
class TestEdgeCases:
    ...
    def test_no_existing_scripts_in_plate(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_malformed_script_names_ignored(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        (
            script_dir / f"{shot_name}_mm-default_{plate_name}_scene_vABC.nk"
        ).touch()  # Non-numeric version
        (
            script_dir / f"{shot_name}_mm-default_{plate_name}_scene_v1.nk"
        ).touch()  # Wrong version format
