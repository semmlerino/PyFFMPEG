...
class TestThumbnailCaching:
    ...
    def test_get_cached_thumbnail_is_persistent(
        ...
        still_valid = cache_manager.get_cached_thumbnail(
            "test_show", "seq01", "shot010"
        )
...
class TestIncrementalShotMerging:
    ...
    def test_workspace_path_change_not_structural_change(
        ...
        cached = [
            Shot(
                "show1", "seq01", "shot010", f"{Config.SHOWS_ROOT}/show1/seq01/shot010"
            )
        ]
        fresh = [
            Shot(
        ...
        assert len(result.updated_shots) == 1
        assert (
            result.updated_shots[0]["workspace_path"]
            == f"{Config.SHOWS_ROOT}/show1/seq01_v2/shot010"
        )
        ...
        assert result.has_changes is False, (
            "Metadata-only updates are not structural changes"
        )
    ...
    def test_merge_performance_linear_time(self, cache_manager: CacheManager) -> None:
        ...
        # Generate 500 shots
        large_cached = [
            Shot("show1", "seq01", f"shot{i:04d}", f"/p{i}") for i in range(500)
        ]
...
class TestShotMigration:
    ...
    def test_migration_cross_show_uniqueness(self, cache_manager: CacheManager) -> None:
    ...
    def test_migration_signal_emission(
        self, cache_manager: CacheManager, qtbot
    ) -> None:
