...
class TestNukeLaunchHandler:
    ...
    def test_handle_workspace_scripts_open_latest(
        ...
        with patch(
            "nuke_launch_handler.PlateDiscovery.find_existing_scripts"
        ) as mock_find:
            mock_find.return_value = [
                (
                    Path(
                        "/test/workspace/comp/nuke/FG01/TEST_0010_mm-default_FG01_scene_v001.nk"
                    ),
                    1,
                )
            ]
    ...
    def test_handle_workspace_scripts_create_new(self, nuke_handler, mock_shot) -> None:
        ...
        with (
            patch(
                "nuke_launch_handler.PlateDiscovery.get_next_script_version"
            ) as mock_get_next,
            patch.object(nuke_handler, "_create_new_workspace_script") as mock_create,
        ):
            mock_get_next.return_value = 2
            mock_create.return_value = (
                "/test/workspace/comp/nuke/FG01/TEST_0010_mm-default_FG01_scene_v002.nk"
            )
