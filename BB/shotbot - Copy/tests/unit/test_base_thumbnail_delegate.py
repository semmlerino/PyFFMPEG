...
class TestSizeHint:
    ...
    def test_size_hint_custom_thumbnail_size(self) -> None:
        ...
        expected_height = (
            custom_size + delegate.theme.text_height + 2 * delegate.theme.padding
        )
    ...
    def test_size_hint_with_custom_padding(self, qtbot) -> None:
        ...
        expected_height = (
            Config.DEFAULT_THUMBNAIL_SIZE + custom_theme.text_height + 2 * 16
        )
...
class TestLoadingAnimation:
    ...
    def test_loading_animation_no_loading_items(self, qtbot) -> None:
        ...
        assert spy.count() == 0, (
            f"Expected 0 signals for no loading items, got {spy.count()}"
        )
    ...
    def test_loading_animation_all_items_loading(self, qtbot) -> None:
        ...
        assert spy.count() == 5, (
            f"Expected 5 signals for 5 loading items, got {spy.count()}"
        )
