...
class TestBundleConfigDefaults:
    """Tests for BundleConfig default values."""

    def test_default_include_patterns(self) -> None:
        """Default config includes common code file patterns."""
        bundler = ApplicationBundler(verbose=False)
        patterns = bundler.config["include_patterns"]

        # Should include common code patterns
        assert "*.py" in patterns
        assert "*.json" in patterns

    def test_default_exclude_patterns(self) -> None:
        """Default config excludes test and cache patterns."""
        bundler = ApplicationBundler(verbose=False)
        patterns = bundler.config["exclude_patterns"]

        # Should exclude test patterns
        assert any("test" in p for p in patterns)

    def test_default_exclude_dirs(self) -> None:
        """Default config excludes certain directories."""
        bundler = ApplicationBundler(verbose=False)
        dirs = bundler.config["exclude_dirs"]

        # Should exclude certain directories
        assert isinstance(dirs, list)

    def test_default_max_file_size(self) -> None:
        """Default config has reasonable max file size."""
        bundler = ApplicationBundler(verbose=False)
        max_size = bundler.config["max_file_size_mb"]
