...
class ProcessConfig:
    ...
    # GPU encoding limits
    MAX_GPU_SLOTS = 12              # RTX 4090 can handle up to 12 encodes (4 per NVENC engine) with 16GB VRAM
    ...
    OPTIMAL_CPU_THREADS = 32        # Optimal thread count for CPU encoding on i9-14900HX (32 threads)
...
class UIConfig:
    ...
    UI_UPDATE_DEFAULT = 400         # Default UI update interval - optimized for high-end system
    UI_UPDATE_HIGH_ACTIVITY = 150   # Fast updates for high activity (4+ processes) - smoother on RTX 4090
    ...
    # Activity timing
    LOW_ACTIVITY_THRESHOLD = 5     # Seconds of inactivity before considering "low activity"
...
class LogConfig:
    ...
    PROCESS_LOG_MAX_SIZE = 10000    # Maximum characters per process log - optimized for 32GB RAM
    PROCESS_LOG_TRUNCATE_LINES = 50  # Lines to keep when truncating process logs
    
    # Log history limits for ProcessManager
    MAX_LOG_HISTORY = 5000          # Maximum log entries to keep in memory - optimized for 32GB RAM
    TRUNCATE_LOG_HISTORY = 2500     # Truncate to this many entries
    
    # Memory cleanup thresholds
    MAX_PROCESS_WIDGETS = 16        # Maximum concurrent process widgets - matches parallel capacity
...
class EncodingConfig:
    ...
    DEFAULT_CRF_H264 = 16           # Default CRF for H.264 - higher quality for powerful hardware
    DEFAULT_CRF_FALLBACK = 23       # Fallback CRF value
    
    # Bitrate settings (in kbps)
    AUDIO_BITRATE_DEFAULT = 256     # Default audio bitrate - higher quality for powerful hardware
