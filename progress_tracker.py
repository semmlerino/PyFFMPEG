...
class ProcessProgressTracker:
    ...
    def __init__(self):
        ...
        self.force_eta_update_interval = 3  # Force ETA update every 3 seconds even with small progress
    ...
    def get_overall_progress(self) -> Dict[str, Any]:
        ...
        if self.total_count > 0:  # Avoid division by zero
            weighted_pct = (process_progress_sum + (self.completed_count * 100)) / self.total_count
        else:
            weighted_pct = 0
        ...
        if weighted_pct > 0:  # Avoid division by zero
            ...
            if self.last_progress_value > 0 and current_time > self.last_progress_time:
                ...
                should_update = (progress_diff > 0.05 and time_diff > 0.5) or time_diff > self.force_eta_update_interval
            ...
            smoothed_eta = sum(eta * (w/total_weight) for eta, w in zip(self.prev_eta_values, weights))
    ...
    def get_process_progress(self, process_id: str) -> Optional[Dict[str, Any]]:
        ...
        if process_id in self.processes:
            ...
            if pct > 0:
                ...
                if process["last_progress_value"] > 0 and current_time > process["last_progress_time"]:
                    ...
                    if should_update:
                        ...
                        current_rate = max(progress_diff / time_diff, 0.0005)  # Ensure some movement
                        
                        # Calculate raw ETA based on current rate
                        raw_eta = (100 - pct) / current_rate
                        
                        # Cap at reasonable maximum
                        raw_eta = min(raw_eta, 3600 * 12)  # Max 12 hours for individual file
                ...
                # Calculate smoothed ETA using weighted moving average
                weights = [i+1 for i in range(len(process["prev_eta_values"]))]  # More weight to recent values
                total_weight = sum(weights)
                smoothed_eta = sum(eta * (w/total_weight) for eta, w in zip(process["prev_eta_values"], weights))
