...
class SettingsPanel(QObject):
    ...
    def _create_codec_settings_group(self) -> QGroupBox:
        ...
        self.preset_combo.setToolTip("Performance preset affects encoding speed vs. quality")
        ...
        self.crf_spinbox.setToolTip("Lower = better quality, larger file (18 recommended)")
    ...
    def _create_performance_settings_group(self) -> QGroupBox:
        ...
        self.auto_balance_checkbox.setToolTip("Automatically distribute files between GPU and CPU encoding")
        ...
        self.priority_combo.setToolTip("Set FFmpeg process priority (High may affect system responsiveness)")
    ...
    def _create_advanced_settings_group(self) -> QGroupBox:
        ...
        self.delete_source_checkbox.setToolTip("⚠️ This will permanently delete the original files!")
        ...
        self.overwrite_checkbox.setToolTip("Overwrite output files if they already exist")
        ...
        self.smart_buffer_checkbox.setToolTip("Optimize memory usage and CPU overhead during conversion")
        ...
        self.hevc_10bit_checkbox.setToolTip("Use 10-bit color depth for HEVC encoding (better quality, larger files)")
    ...
    def _connect_signals(self) -> None:
        ...
        required_controls = [
            self.codec_combo, self.preset_combo, self.hwdecode_combo, self.crf_spinbox,
            self.threads_spinbox, self.parallel_checkbox, self.max_parallel_spinbox,
            self.delete_source_checkbox, self.overwrite_checkbox, self.smart_buffer_checkbox,
            self.auto_balance_checkbox, self.priority_combo, self.hevc_10bit_checkbox
        ]
    ...
    def _restore_settings(self) -> None:
        ...
        saved_settings = {
            ...
            "preset_idx": self.settings.value("preset_idx", 2, type=int),  # Default to Fast
            "hwdecode_idx": self.settings.value("hwdecode", 0, type=int),  # Match original key
            "crf_value": self.settings.value("crf", EncodingConfig.DEFAULT_CRF_H264, type=int),  # Match original key
            "threads": self.settings.value("threads", min(ProcessConfig.OPTIMAL_CPU_THREADS, os.cpu_count() or 4), type=int),
            "parallel_enabled": self.settings.value("parallel_enabled", True, type=bool),  # Default enabled for high-end
            "max_parallel": self.settings.value("max_parallel", 4, type=int),  # Default 4
            "delete_source": self.settings.value("delete", False, type=bool),  # Match original key
            "overwrite_mode": self.settings.value("overwrite", True, type=bool),  # Match original key
            "smart_buffer": self.settings.value("smart_buffer", True, type=bool),
            "auto_balance": self.settings.value("auto_balance", True, type=bool),  # Default enabled
            "priority_idx": self.settings.value("priority_idx", 0, type=int)  # Default to Normal
        }
    ...
    def validate_settings(self) -> tuple[bool, str]:
        ...
        if settings.get("parallel_enabled") and settings.get("max_parallel", 1) > ProcessConfig.MAX_PARALLEL_HIGH_END:
            return False, f"Maximum parallel processes cannot exceed {ProcessConfig.MAX_PARALLEL_HIGH_END}"
    ...
    def _show_nvenc_settings(self) -> None:
        ...
        ref_frames_spinbox.setToolTip("Number of reference frames (higher = better quality, more VRAM)")
        layout.addRow("Reference Frames:", ref_frames_spinbox)
        
        # Rate control mode
        rc_mode_combo = QComboBox()
        rc_mode_combo.addItems(["VBR (Variable)", "CBR (Constant)", "CQP (Constant QP)"])
        rc_mode_combo.setCurrentIndex({"vbr": 0, "cbr": 1, "cqp": 2}.get(self.nvenc_rc_mode, 0))
